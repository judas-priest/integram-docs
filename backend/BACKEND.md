...
| Модуль | Описание |
|--------|----------|
| **ai** | Крупнейший модуль. LLM-прокси, 24 агента, 726 инструментов, автозаполнение, AI-колонки, NL→SQL, HITL, quality worker, background scans. Подробнее: [AI-агент](#ai-агент) |
...
Пользователь → POST /ai/agent-chat (SSE)
    ...
    (runner.js)   (runner.js) (runner.js)
         ↓         ↓         ↓
    tool-executor.js (switch 726 cases + workspace tools fallback)
         ↓
