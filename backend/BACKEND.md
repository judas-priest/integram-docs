...
| Модуль | Описание |
|--------|----------|
| **ai** | Крупнейший модуль. LLM-прокси, 24 агента, 765 инструментов, автозаполнение, AI-колонки, NL→SQL, HITL, quality worker, background scans. Подробнее: [AI-агент](#ai-агент) |
...
| **presentations** | PPTX-движок: каноническая JSON-модель слайдов, экспорт .pptx и PDF-превью, импорт .pptx, версии-снапшоты, шаринг. Судья доступа checkPresAccess общий для REST и ИИ |
...
| **finmodel** | Финмодели: расчётные группы, ось периодов (год/квартал/месяц/неделя), сценарии, формулы со ссылками на строки и сдвигом по периодам, граф зависимостей ПО ЯЧЕЙКАМ (самоссылка со сдвигом — не цикл), подбор параметров (goal-seek) с записью решения. Модуль области `finmodel`, по умолчанию включён |
...
`_v2_autofields`, `_v2_computed_reqs`, `_v2_audit_log`, `_v2_views`, `_v2_comments`, `_v2_webhooks`, `_v2_notifications`, `_v2_row_rules`, `_v2_automations`, `_v2_files`, `_v2_type_meta`, `_v2_schema_audit`, `_v2_schema_snapshots`, `_v2_report_audit`, `_v2_entity_meta`, `_v2_documents`, `_v2_doc_blocks`, `_v2_embedding_sync`, `_v2_column_validation`, `_v2_record_mentions`, `_v2_timeseries`, `_v2_template_origin`

`_v2_template_origin` — карта номеров шаблона (ADR-029): `(template_key, section,
local_id) → real_id`, где `template_key` — СЛАГ шаблона (он переживает пересъёмку
эталона, номер — нет). По ней повторное наложение шаблона узнаёт СВОИ строки и
правит их на месте вместо того, чтобы заводить вторые. Пишут её оба пути
применения (`applyTemplateToWorkspace`, `createWorkspaceFromTemplate`); шаблоном
она не едет — номера в другой области значат другое.
...
Пользователь → POST /ai/agent-chat (SSE)
    ...
    (runner.js)   (runner.js) (runner.js)
         ↓         ↓         ↓
    tool-executor.js (switch 765 cases + workspace tools fallback)
         ↓
