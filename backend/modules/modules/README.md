...
| Module | File | Description |
|--------|------|-------------|
| [ai](ai.md) | `modules/ai/` | Multi-agent chat (SSE), 24 agents, 765 tools (platform + per-workspace custom), autofill, AI columns, MCP, HITL |
...
| [files](files.md) | `modules/files/` | File upload/download (500 MB), PDF/DOCX parsing, OCR, embedding |
| [presentations](presentations.md) | `modules/presentations/` | PPTX-движок: каноническая модель слайдов, экспорт/импорт .pptx, версии, шаринг |
| [video-engine](video-engine.md) | `modules/video-engine/` | Обучающие видео: сценарий (JSON) → screencast headless chromium → TTS (edge-tts) → монтаж (ffmpeg) → mp4 |

### Automations and Integrations

| Module | File | Description |
|--------|------|-------------|
| [automations](automations.md) | `modules/automations/` | Event-driven rules (trigger types — see automations.md), BullMQ, cron, AI insights |
| [meetings](meetings.md) | `modules/meetings/` | Конвейер встреч: TG-запись → расшифровка → протокол → одобрение → PM-задачи; per-workspace конфиг, устанавливает свои автоматизации |
...
| [script-button](script-button.md) | `modules/script-button/` | Script Button column (type 1020): user-written JS runs in the browser (Web Worker) on click, row data + fetch available |
...
| [finmodel](finmodel.md) | `modules/finmodel/` | Financial models: calculation groups, periods (year/quarter/month/week), scenarios, cell-level formula graph, goal seek |
| [synced-lookups](synced-lookups.md) | `modules/synced-lookups/` | External lookup: pull-synced read-only copy of a table from another workspace (rename-safe mapping, access re-check, BullMQ cron) |
...
| [error-collector](error-collector.md) | `modules/error-collector/` | Frontend/backend error collection with fingerprint dedup + MCP bug-report intake (`report_platform_issue` → PM task in feedback workspace) |
