...
POST /ai/agent-chat
   ...
   - Mid-turn context compression: prune → merge → LLM-summarize (swarm-extract) при превышении 20000 токенов
        ↓
   tool-executor.js (726 switch cases)
...
| `portal` | 37 | Portal config, publish, orders, metrics, client profiles, custom Vue components |
...
посмотрел. Полные карточки инструментов ограничены 25 на вызов — 726 схем
