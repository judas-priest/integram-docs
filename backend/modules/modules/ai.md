...
POST /ai/agent-chat
   ...
   - Mid-turn context compression: prune → merge → LLM-summarize (swarm-extract) при превышении 20000 токенов
        ↓
   tool-executor.js (765 switch cases)
...
| `portal` | 44 | Portal config, publish, orders, metrics, client profiles, custom Vue components |
...
| `teamchat` | 33 | Team discussions, decisions, codespace integration |
| `telegram` | 27 | Telegram bot management, messages, moderation, payments, stories |
| `codespace` | 26 | Git repositories, branches, commits, files, PRs, GitHub Sync |
| `admin` | 23 | Members, settings, backups, trash, notifications, audit, workspace info, templates |
...
- Frontend: `POST /ai/agent-chat` can include `enabledAgents` in the body → `ctx.enabledAgentSlugs` → copied to `toolCtx` in `runAgent`
- Workspace agents list for the UI: `GET /:db/meta-kb/agents` (fields whitelisted, no `system_prompt`)
...
### Report WHERE: sanitizeWhere rules

`update_report` (and the REST `PATCH /reports/:id`) validate the `where` fragment through `sanitizeWhere` (`backend/src/api/v2/utils/report-engine.js`). Rules the model and integrators must follow:

- Subqueries are allowed. The workspace EAV table inside a subquery must be referenced **only** through the `{{DB}}` placeholder (resolved to the current workspace); a foreign schema or unqualified system table is rejected.
- A subquery alias must match `/^_[a-z]+$/i` — `AS _sub` is valid, `AS sub` is rejected (`c<id>` and `a` are engine aliases; `alias.<number>` references a requisite id and is allowed).
- Forbidden: `;`, SQL comments (`--`, `/* */`), `$$` dollar-quoting, disallowed keywords/functions, `public.` / `information_schema` / `pg_catalog` references.

Example of a valid filter: `AND c123.val IN (SELECT id FROM {{DB}} _t WHERE _t.t = 3)`.

### Tool error envelope over HTTP (TD-065)

Tool execution failures on `POST /ai/tool`, `/ai/mcp-resume` and `/ai/agent-resume` return an honest HTTP error envelope — `4xx` + `{ ok:false, error: { code, message, details? } }` — the same shape as the rest of API v2. Earlier these endpoints answered `200` with `{ ok:true, data: { error:true, ... } }`; clients that checked only `data.error` must switch to the status code / `ok` field. The MCP proxy converts the failure into an `isError: true` tool result (spec-compliant) and keeps the legacy `data.error` conversion as a fallback for older backends.
...
посмотрел. Полные карточки инструментов ограничены 25 на вызов — 765 схем
