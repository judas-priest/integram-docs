...
POST /ai/agent-chat
   ...
   - Mid-turn context compression: prune → merge → LLM-summarize (swarm-extract) при превышении 20000 токенов
        ↓
   tool-executor.js (300+ switch cases)
...
| `reports` | 15 | Report create/edit, columns, filters, aggregation, run |
...
| `portal` | 15 | Portal config, publish, orders, metrics, client profiles, custom Vue components |
| `normalizer` | 5 | Normalization pipeline: start, status, confirm |
| `timeseries` | 3 | Record and query timeseries data |
| `advisor` | 4 | Platform usage advice |
...
| `codespace` | 23 | Git repositories, branches, commits, files, PRs, GitHub Sync |
| `meta-kb` | 22 | Meta KB mode: debate engine, streaming 3-phase analysis (research→debate→synthesis), knowledge curation, KAG entity management, change requests |
| `sandbox-researcher` | 5 | Research agent for sandbox: RAG knowledge base, decision search, teamchat search, table data |
| `sandbox-engineer` | 3 | Code generation agent: generates code, sends results as code cells to teamchat |
...
POST /codespace/review-gate { repo, pr }   (зовёт инфраструктура по событию PR — контракт шва B3/B4)
   ...
   - структуризация в вердикт по схеме A2 (review-verdict.js), 1 ретрай,
     невалидный вердикт в карточку не уходит (REVIEW_GATE_INVALID_VERDICT)
        ↓
   { verdict, evidence }  — verdict: контракт A2 (docs/review-verdict-schema.md),
