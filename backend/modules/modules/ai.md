...
POST /ai/agent-chat
   ...
   - Mid-turn context compression: prune → merge → LLM-summarize (swarm-extract) при превышении 20000 токенов
        ↓
   tool-executor.js (740 switch cases)
...
| `portal` | 38 | Portal config, publish, orders, metrics, client profiles, custom Vue components |
...
| `admin` | 22 | Members, settings, backups, trash, notifications, audit, workspace info, templates |
...
посмотрел. Полные карточки инструментов ограничены 25 на вызов — 740 схем
