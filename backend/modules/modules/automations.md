...
| GET | `/automations/:id/runs` | any | Execution history (limit/offset). Each run carries `resultData`: `{ output, logs }` — the `output()` value and captured `console.log` lines of run_script actions |
...
| `on_file_processed` | File processing reached a terminal status (bus `file.processed`). Optional `status` filter: `done`\|`error`\|`skipped`\|`confirmed` — empty fires on any terminal status. Payload: `fileId`, `objectId`, `status`, `error`, `filename`. |
| `on_member_joined` | A new member joined the workspace (bus `member.joined`, emitted by invite-link acceptance after the membership transaction commits). ctx: `memberUserId`, `memberEmail`, `memberRole`, `workspaceSlug`, `meta`, `invitationId`. |
...
`update_field`, `set_requisite`, `set_field` (alias for `set_requisite`), `update_related`, `update_related_records`, `create_object`, `delete_object`, `send_notification`, `send_notification_to_group`, `fire_webhook`, `http_request`, `run_connector`, `send_email`, `send_sms`, `send_telegram`, `send_telegram_media`, `create_document`, `delete_telegram_message`, `send_chat_action`, `run_ai_agent`, `run_script`, `run_server_function`, `run_video_job`, `request_approval`, `if_else`, `switch`, `transform`, `wait_delay`, `delegate_to_agent`, `invoke_agent`, `create_cdek_shipment`, `reconcile_cdek`, `create_teamchat_room`, `telegram_forward`, `send_invoice`, `answer_inline_query`, `telegram_ban`, `telegram_unban`, `telegram_restrict`, `telegram_promote`, `telegram_approve_join`, `telegram_decline_join`, `telegram_pin`, `telegram_unpin`, `telegram_get_chat`, `telegram_download_file`, `telegram_post_story`, `answer_shipping`, `telegram_business_reply`, `create_issue`, `update_issue`
...
- other fields (all optional): `description`, `priority` (default `medium`), `assignee_id`, `reporter_id`, `parent_id`, `board_id`, `sprint_id`, `due_date`, `estimate`, `labels` (comma-separated string)
- `reporter_id` (PM-643) — numeric user id; overrides the running user as reporter. Without it a Telegram-triggered automation falls back to the workspace owner (who becomes a watcher of every machine-created issue). Note: `reporter_id` is automations-only — the REST create route and `pm_bulk_update` strip it (`issueCreateBody` transform in `pm/router.js`, field filter in `ai/agent/tools/pm.js`); `PATCH /issues/:id` also never accepted it.
- outputs: `{{_created_issue_id}}`, `{{_created_issue_number}}`

#### Role routing (`assigneeRole`)

Instead of a literal `assignee_id`, the issue can be routed to a member of a role:

```json
{
  "type": "create_issue",
  "title": "Анализ заявки: {{val}}",
  "assigneeRole": "Аналитики",
  "assigneeStrategy": "least_busy"
}
```

- `assigneeRole` — name of a workspace EAV role-group table: its rows are the role
  members, each carrying an Integram login in a column named `Логин` / `Username` /
  `Email` (case-insensitive). The column is matched by display name OR alias —
  `add_column` stores the display name in the `:ALIAS=` modifier and `name` may be
  empty, so a column created through the schema API is still found. This is the same kind of group `send_notification_to_group`
  uses — workspace membership roles (`owner`/`admin`/`editor`/`viewer`) and `_v2_roles`
  entries have no user→role assignment and cannot express functional roles.
- `assigneeStrategy` (optional, default `least_busy`):
  - `least_busy` — the member with the fewest open issues (status kind is not
    `done`/`canceled`, board-aware); ties broken by lower user id;
  - `first` — the first member in the role table order.
- `assignee_id` and `assigneeRole` are mutually exclusive — an automation carrying
  both is rejected with a validation error at save time.
- Fallback: if the role table is missing, has no login column, no members, or none
  of the logins resolve to a user, the issue is created **without an assignee** and
  a notification is sent to all role members (or to the triggering user when the
  role itself was not recognized). The run is not failed and not silently skipped.
...
- optional fields: `title`, `description`, `status`, `priority`, `assignee_id`, `reporter_id`, `sprint_id`, `parent_id`, `estimate`, `due_date`, `labels`, `issue_type`
- `board_id` — moves the issue to another board via `moveIssueToBoard`
  (`pm/service.js`), which re-picks the status by board kind. Retries after a
  partial success are safe: VALIDATION_ERROR «Задача уже на этой доске» (400)
  is swallowed as success, any other error propagates.

Reporter/actor convention for both actions: `create_issue` passes the running
user's id as `reporter_id`; when the run has no user with a numeric id
(schedule runs, synthetic `tg:*` users), reporter is `null` and `pm/service.js`
falls back to the workspace owner (fst fix 2026-09-08) — never `0`, which would
orphan the issue. History entries (`pm_issue_history.actor_id`) use `0` for
system runs, per the `pm/listeners.js` convention.
...
**Script API:**
- `row` — record fields as `{ ID, parentId, FieldName: value, ... }` (loaded from EAV if objId is set; empty `{}` otherwise). `parentId` is the parent object ID (null for root-level records). **Do NOT declare `const row` in a script** — it shadows this global, and any read before the declaration throws `ReferenceError` (TDZ). Use another name (`const rec = ...`).
- `objId`, `typeId` — trigger record/type IDs (`null` when there is none). In `on_create` and other record triggers the script has no `ctx` — these globals and `row` are the whole context.
- `SHAPES` — a reference string listing every data-read shape below. `console.log(SHAPES)` prints it into the run log.
...
- `fetch(url, opts)` — HTTP request, returns `{ status, body, ok }`. **`body` is already the response TEXT** — there is no `resp.text()` / `resp.json()`; parse JSON with `JSON.parse(resp.body)`. SSRF-protected: localhost, private IPs, `.local`, `.internal` are blocked.
- `ai(prompt, model?)` — LLM call (model: `"fast"` default), returns response text
- `query(typeId, opts)` — list records. Rows carry `{ id, value, parentId, typeId, order }` plus field values keyed by column NAME (`:ALIAS=` wins) AND by numeric reqId. A column named like a metadata key lands under `<name>_` (e.g. `parentId_`). Pass `opts: { reqs: '1' }` for the legacy numeric-only shape. `typeId` accepts the type NAME as well as its number. Truncation above `pageSize` (default 50, max 1000) is silent.
- `getRecord(id)` — `{ id, value, typeId, requisites: { reqId: value } }` with the same values also under column names: `rec['Статус']` works; `rec.requisites['305']` is the raw numeric-keyed map.
...
- `_script_result` — `null` on error (explicitly set — comparisons against it refuse per the TD-075 rule, `[_script_error]` stays available for optional branching)
- `_script_logs` — joined log lines (only set when logs exist)
- `_script_error` — error message (only set on failure; `_script_logs` is NOT set when error occurs, `_script_result` is `null`)

**Ref columns — read and write (PM-645):**

- Read (`query`/`queryPage`/`getRecord`): a ref cell is the string `"id:Имя"` — e.g. `"408:ООО РоботехПро"`. The numeric id is always the part before the FIRST colon, so `String(v).split(':')[0].trim()` is a safe id extraction even when the record name itself contains colons.
- Write (`updateRecord`/`createRecord`): pass the numeric id — `680` or `'680'` (the bridge stringifies every field value, both forms are identical). A display form like `"АО МикроСенс (id:395)"` or a bare name is NOT resolved here: `parseInt` fails and the column is skipped silently. Writing by name is available on the AI-tool path (`create_object`/`update_object`), not in the sandbox.

**Retries (PM-644):** a BullMQ retry resumes from the checkpoint of the last fully successful action. A run_script whose script failed does NOT advance the checkpoint — the retry re-runs the script, and actions AFTER it re-run too (a send_telegram/send_email below a failed script can fire again on retry — same semantics as any thrown action). Script errors are prepended to the run error, never overwritten by a later action's error.
...
Медиа в Telegram: голосовое (текст → Piper TTS → ffmpeg OGG; без ffmpeg — WAV документом), видео готовой джобы video-engine ({{_video_job_id}} — после действия run_video_job), файл по URL. chatId/text/caption принимают {{переменные}}.

```json
{
  "type": "send_telegram_media",
  "kind": "voice",
  "botId": 42,
  "chatId": "{{req_501}}",
  "text": "Итоги совещания: {{req_328}}",
  "caption": "",
  "videoJobId": "",
  "fileUrl": "",
  "resultVar": "tgMsg"
...
- `kind` — `voice` | `video` | `document`; по умолчанию `voice` (UI показывает `voice` и может не сохранить kind).
- `voice`: `text` синтезируется Piper'ом, PCM упаковывается в OGG через ffmpeg; при недоступном ffmpeg голосовое уходит как WAV-файл документом.
- `video`: `videoJobId` — id джобы video-engine, отправляется её `resultFile`.
- `document`: `fileUrl` — файл скачивается и отправляется как документ.
- `botId` опционален — иначе берётся `ctx._tgBotId`.

**Environment variables set after execution:**
- `{{_tg_media_message_id}}` — id отправленного сообщения (не выставляется при неудаче)
- `{{_tg_media_error}}` — причина неудачи

### create_document

Документ воркспейса из markdown (markdownToBlocks → documents service). Протокол совещания появляется на портале документом. Владелец: `user.id` контекста или `created_by` автоматизации (иначе отказ — документ без владельца невидим никому).

```json
{
  "type": "create_document",
  "title": "Протокол {{NOW}}",
  "markdown": "{{_script_result}}",
  "resultVar": "docId"
}
```

**Environment variables set after execution:**
- `{{_document_id}}` — id созданного документа (не выставляется при неудаче)
- `{{_document_error}}` — причина неудачи (`empty markdown`, `markdown too large`)

### run_video_job

Постановка джобы video-engine: рендер ролика по сценарию (steps[] + текст озвучки, конвейер tts→record→assemble живёт в modules/video-engine). Не ждёт рендер: jobId в {{_video_job_id}} / {{resultVar}}, статус и mp4 — маршруты /video-engine/jobs/:id. Отказ: {{_video_job_error}}.
...
Action fields (value, message, requisites values, parentId) support `{{variable}}` placeholders resolved at runtime via `resolveVal()`. Keys are env variable names: `{{id}}`, `{{val}}`, `{{typeId}}`, `{{req_N}}` for field N, `{{_raw_req_N}}`, `{{_script_result}}` / `{{_script_logs}}` / `{{_script_error}}` after a run_script action, `{{NOW}}`, `{{TODAY}}`, `{{NOW_ISO}}`, `{{TODAY_ISO}}`, `{{created_by}}`. A numeric-only placeholder (`{{5}}`) also resolves to field 5's value when that field exists. An unknown key silently becomes an empty string.
...
| `{{file_id}}`, `{{file_object_id}}`, `{{file_status}}`, `{{file_error}}`, `{{file_name}}` | File context (for `on_file_processed` triggers; empty fields come through as `''`) |
| `{{memberUserId}}`, `{{memberEmail}}`, `{{memberRole}}`, `{{workspaceSlug}}`, `{{meta}}` | Member context (for `on_member_joined` triggers; `meta` is a JSON string) |
...
| `[Имя]` with no such variable in the environment — or a variable whose value is `null`/`undefined` (no value is no variable) | `в окружении нет переменной «…»` |
...
- `member.joined` → enqueues `on_member_joined` automations. Emitted by: invite-link acceptance (`acceptInvitation`, after the membership transaction commits)
