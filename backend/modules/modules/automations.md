...
**Files:** `router.js`, `service.js`, `worker.js`, `listeners.js`, `isolated-runner.js`, `dispatchers.js`, `execution.js`, `ai-analysis.js`, `scheduler.js`, `batch.js`, `lifecycle.js`, `telegram-helpers.js`, `helpers.js`
...
- `createRecord(typeId, { name, fields })` — create record, returns new ID. `fields` = `{ reqId: value }`
...
**Limits:** 128 MB RAM, 60s timeout, 30 query / 50 mutation / 50 fetch / 10 AI calls per execution.
...
- `formula`: expression using `[FieldName]` column references (formula engine)
