...
| DELETE | `/objects/:objId/comments/:commentId` | Delete comment (own or admin) |
...
Reaction {
  comment_id, emoji (max 8 chars), user_id, created_at
}
```
...
On `createComment`: emits `comment.created` with `{ db, objId, commentId, author }`.

## DB Tables

- `_v2_comments` (per-workspace) — comment threads
- `_v2_comment_reactions` (per-workspace) — emoji reactions per comment per user
