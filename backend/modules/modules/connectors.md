...
Коннектор — две строки EAV:

- контейнер: корневая запись `(up=1, t=0, val=<имя>)`. `t=0` — generic-тип:
  сканы по типу его не отдают (фильтр `a.t = ?`), а `up=1` выводит его из
  подзапроса маркеров в `objectWhere` (`shared/sql-guards.js`) — без этого
  дочерний конфиг невидим `query(226)`/`list_objects` (PM-493);
- конфиг: ребёнок `(up=<id контейнера>, t=226 /* TYPE.CONNECT, константа
  глобальная */, val=<JSON>)`.

Модульные читатели (`listConnectors`, `getConnector`, `executeConnector`) ищут
коннекторы только по паре «контейнер + конфиг-ребёнок t=226» — `up`/`t`
контейнера не читают. Коннекторы, созданные до 16.09.2026, переводит
`backend/scripts/fix-connector-markers.mjs` (холостой прогон по умолчанию).
...
## Config validation on save (#255)

`createConnector`/`updateConnector` валидируют конфиг ДО любой записи
(`config-validation.js` → `parseConfigStrict` + `assertConfigValid`):

- resolve-ключи (`typeId`, `byReqId`, `create`, `createName`, `createNameField`,
  `valueMap`, `stripAfterColon`) на верхнем уровне записи маппинга — ошибка:
  читаются только внутри `resolve` (`resolveRef`);
- `resolve` обязан быть объектом с `typeId` и `byReqId` (не null);
- неизвестные ключи записи маппинга — ошибка (перечень: `reqId`, `path`, `fn`,
  `epoch`, `invert`, `skipIfExists`, `priorityMap`, `priorityAlwaysAccept`, `resolve`);
- `relations` — массив, если задан;
- невалидная JSON-строка конфига — `VALIDATION_ERROR`, а не пустой конфиг;
  пустая строка по-прежнему значит «конфига нет».

Валидируются только Mode 1 конфиги (`targetTypeId` задан); Mode 2 (маппинг
ответа в объект, как в пресетах) — другой контракт, не проверяется.

Прогон возвращает `imported.refResolution` — по каждой ссылочной колонке
`{attempted, resolved, skippedNoValue, skippedUnresolved}` (relations — с ключом
`<targetTypeId>:<reqId>`) и warning, когда колонка не записала ничего.

Скан существующих конфигов перед выкладкой: `node scripts/scan-connector-configs.mjs`
(только чтение; код 0 — чисто, 1 — есть конфиги, которые сохранение после
выкладки отвергнет, 2 — не смог измерить целиком: ошибка базы, пустой перечень
воркспейсов). Воркспейс без собственной схемы пропускается (это замер, а не
пробел).
