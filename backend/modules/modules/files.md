...
**Files:** `router.js`, `service.js`, `paths.js`, `doc-processor.js`, `ocr-chain.js`, `audio-transcriber.js`, `paddle-ocr.js`
...
| POST | `/files` | Upload file (multipart/form-data, max 500 MB) |
...
1. `multer` streams the multipart upload to a temp file on disk (`upload-middleware.js`,
   `os.tmpdir()/integram-uploads`); the limit is `MAX_UPLOAD_BYTES` (500 MB, `limits.js`) —
   anything larger is refused with `FILE_TOO_LARGE` / 413. Nothing holds the upload in RAM:
   MIME checks read a head chunk (`readHead`), the temp file is MOVED into the upload dir
   (`service.storeFile`, which also removes the staged file on any failure). The buffer-based
   `storeFileBuffer` remains for callers that genuinely hold bytes (telegram_download_file
   automation).
...
4. Temp file moved into workspace upload directory by `service.storeFile` (respects optional `subdir`)
...
Uploads are allowed up to 500 MB (`MAX_UPLOAD_BYTES`) and the document travels as a base64
string in the request body, so without the guard a worker would hold a ~660 MB string for a
request the provider rejects anyway.
...
**File size limit:** 100 MB. Files exceeding this limit fail with an error stored in `processing_error`.
...
3. Transcript saved to `extracted_text`, `processing_status` set to `done`. An empty transcript is a provider refusal, not a processed file: the status becomes `skipped` with `processing_error` = *«Расшифровка не дала текста: Whisper вернул пустой ответ»*, and steps 4–5 are not reached
...
### Text written back to the linked record

The transcript is not the only text that travels back to the object the file hangs on.
Recognized/extracted text does the same, into a memo column aliased **`"Распознанный текст"`**
(`ALIAS_RECOGNIZED` in `doc-processor.js`), through the same `deliverTextToObject` helper and
on all three branches that produce text: a vision-LLM answer for an image, a vision answer for
a scanned PDF, and the end of the ordinary extraction pipeline. Symmetrical to the transcript —
otherwise recognition is visible only in the file card and never in the record.

`deliverTextToObject` is silent by design: no `object_id`, empty text, or no column with that
alias in the type definition means nothing is written (a failed write is logged, never thrown).

After a write that actually changed the value, `writeTextToObjectField` dispatches an
`on_update` automation event (`changedReqIds: [reqId]`, actor `doc-processor`) — the write
goes through direct SQL past `objects/service.js`, so without the explicit dispatch object
rules never saw the transcript land. If the new text equals the stored value, neither the
write nor the dispatch happens (repeat processing of the same file must not re-fire rules).
The dispatch failure is logged, never thrown. Same shape as the connector dispatch
(`connectors/cdek/actions.js`); the resulting `automations↔files` import cycle is a known
entry in `ai/agent/__tests__/module-cycles.test.js`.

## Event `file.processed` in the event bus (TD-069)

Every `processing_status` change flows through the single funnel
`updateFileProcessing` (`service.js`). When the new status is terminal —
`done`, `error`, `skipped` or `confirmed` — the funnel emits a process-local
bus event:

```
bus.emit('file.processed', { db, pool, fileId, objectId, status, error, filename })
```

`objectId`, `filename` (`original_name`) and `error` (`processing_error`) come
from the `RETURNING` clause of the funnel's UPDATE. Intermediate statuses
(`pending`, `extracting`, `classifying`) are progress, not readiness — they are
never emitted. `confirmExtractedFields` writes `confirmed` through the same
funnel, so confirming extracted fields fires the event too.

`automations/listeners.js` subscribes to this event and dispatches
`on_file_processed` automations (see `automations.md` for the trigger type and
`{{file_*}}` template variables).

Caveats:

- **Process-local bus.** The doc-processor worker runs inside the HTTP process
  (`startDocProcessorWorker` from `api/v2/index.js`), the same process where
  automation listeners register — so the emit is heard. If file processing is
  ever moved to a separate worker process (like the normalizer), listeners go
  blind silently; use `crossPublish` (see `utils/ws-listeners.js`) at that point.
- **A file stuck in `extracting`** after a process crash is never reprocessed
  (`getPendingFiles` selects only `pending`), so its terminal event never fires.
  Existing behaviour, not introduced by the event.
- **A second `error` emit** is admissible: the outer catch in
  `doc-processor.js` can write `error` again if the funnel itself throws.
  Automation runs are idempotent BullMQ jobs — same semantics as every other
  trigger.
...
records metadata and routes the file into the processing pipeline. It has a path-based twin,
`service.storeFile` (above): the HTTP route has already streamed the upload to a temp file,
so `storeFile` moves that file into place instead of buffering bytes. `storeFileBuffer`
itself remains for callers that genuinely hold bytes in memory — the `telegram_download_file`
automation action — a file must reach OCR/transcription the same way no matter how it arrived.

**It is not yet the only path — but the HTTP doors now share one.** The four upload routes
below accept the multipart upload through the same `makeDiskUpload()` (`files/upload-middleware.js`)
and store the file through `filesService.storeFile`:

| Place | What remains its own |
|---|---|
| `portal/router.js` — `POST /api/upload` | response shape (URL); portal-specific upload root (`process.env.UPLOAD_DIR`) |
| `teamchat/router.js` — `POST /topics/:topicId/upload` | response shape (message card) |
| `portal/teamchat/router.js` — `POST /topics/:topicId/upload` | response shape (message card) |
| `ai/router.js` — `POST /upload` | response shape (chat attachment) |
| `ai/agent/tools/excel.js:37` | own copy of the write + `recordFileMetadata` sequence — not an HTTP route |

The upload limit is now one number for all five doors: `MAX_UPLOAD_BYTES` (500 MB, `files/limits.js`).
Previously the four alternative routes each declared `fileSize: 50 * 1024 * 1024` in their own
`multer` instance, so the very same file was accepted through `/files` and refused with 413
through the portal, teamchat or AI upload. The portal/teamchat upload was worse: its own
`getUploadDir` barred role `member` via `repoGrant`, so **every** portal user got 403 —
fixed by moving to `uploadDirFor` with a service user for the non-member portal client.
...
before megabytes are staged to the temp dir. Resolving the directory itself lives in `files/paths.js`
(`uploadDirFor`), apart from the permission check, so the upload root can be substituted
in tests. The portal teamchat upload applies this ordering literally: the topic-membership
gate runs BEFORE multer staging (`portal/teamchat/router.js`), so a non-member cannot
push bytes to tmp at all.

## Reconcile — self-healing sweep

The pending-documents sweep heals its own precondition: `reconcileWorkspacePending`
(`files/doc-processor.js`) probes for the `_v2_files` table first and, on the
"column/table does not exist" error (42703), runs `ensureFilesTable` and retries once
instead of failing the sweep. Error classes are decided locally (fatal vs healable),
each workspace keeps a separate heal-retry key, and the helper is invalidated together
