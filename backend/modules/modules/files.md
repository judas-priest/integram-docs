...
| POST | `/files` | Upload file (multipart/form-data, max 100 MB) |
...
1. `multer` receives the multipart upload (in-memory buffer up to 100 MB; anything larger is refused with `FILE_TOO_LARGE` / 413)
...
|---|---|
| `portal/router.js` — `POST /api/upload` | takes the upload dir from `process.env.UPLOAD_DIR`, not `getUploadDir` |
| `teamchat/router.js` — `POST /topics/:topicId/upload` | own copy of the write-to-disk + `recordFileMetadata` sequence; processing itself is queued through `filesService.queueFileProcessing` |
| `portal/teamchat/router.js` — `POST /topics/:topicId/upload` | same |
| `ai/router.js` — `POST /upload` | own copy of the sequence |
| `ai/agent/tools/excel.js:37` | own copy of the sequence |

**The 100 MB limit is `POST /files` only.** All four alternative upload routes above
declare `fileSize: 50 * 1024 * 1024` in their own `multer` instance, so the very same
file is accepted through `/files` and refused with 413 through the portal, teamchat or
AI upload — with no hint that the ceiling depends on which door you used. Established
by reading the four `multer({ ... })` calls, not inferred.
