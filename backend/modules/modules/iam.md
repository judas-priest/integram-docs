...
| POST | `/iam/register-with-invite` | None | 5/min/IP | Register by one-time invite link (`{ token, email?, password, name?, username? }`) — see Register with invite below |
...
| GET | `/iam/sessions` | JWT | — | List live sessions (grouped by refresh family; cap 100 + `total`; prunes expired first). `?offset=` — offset paging for the «показать ещё» button |
...
`POST /iam/register-with-invite` (`registerWithInvite` in `service.js`) —
регистрация по одноразовой пригласительной ссылке: peek → register → accept.
Тело: `{ token, email?, password, name?, username? }`. До создания аккаунта
выполняется peek (`workspaces`): погашенный или истёкший токен — 404, аккаунт
не создаётся.

Почта:

- приглашение с адресом **фиксирует** его: другой `email` в теле — 400; поле
  можно не передавать вовсе;
- ссылка без email **требует** адрес в теле (400 без него).

Успех — `201 { ok, accessToken, user, workspaceSlug }` + refresh-cookie
(тот же путь, что у `register`). Rate limit как у register (5/min/IP).

Гонка peek→accept: токен могут погасить (принять/отозвать) между peek и
accept. Тогда аккаунт **создан**, но членства нет — ответ `409
«Аккаунт создан (…email…), но приглашение не принято: …»`; оба факта названы,
тихого успеха нет. Повторная попытка тем же токеном — 404.
