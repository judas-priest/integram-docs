...
- **Meta KB agent** (`agents/meta-kb.js`): separate agent with 25 core tools, streaming 3-phase analysis (research → debate → synthesis), cross-debate streaming, markdown rendering
...
Creates `_v2_mk_debates` table and indexes (`idx_mk_debates_db`, `idx_mk_debates_topic`) if they do not exist. Memoized — only runs DDL once per process lifetime via an internal `_tablesReady` flag.
...
### `linkDebateToDecision(pool, db, topicId, decisionId, createdBy)`
Creates an iteration record linking all debates for a topic to a decision. Used when a debate outcome is fixated as a decision.
...
Additional non-mk tools available to the meta-kb agent: `create_object`, `update_object`, `list_documents`, `create_document`, `append_block`, `get_report`, `list_reports`, `create_topic`, `update_topic`.
