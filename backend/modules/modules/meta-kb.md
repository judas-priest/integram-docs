...
backend/src/api/v2/modules/ai/agent/
  ...
  tools/meta-kb.js   — 25 mk_* tool handlers
```
...
- **`agent/index.js`** (TOOL_DEFS): all 25 mk_* tools registered (`mk_start_debate`, `mk_analytics`, `mk_research`, `mk_propose_change`, `mk_revoke_entity`, `mk_list_debates`, `mk_welcome`, `mk_list_rules`, `mk_run_rules`, `mk_gift_matrix`, `mk_gift_closed`, `mk_create_rule`, `mk_delete_rule`, `mk_list_iterations`, `mk_get_debate`, `mk_list_snapshots`, `mk_create_snapshot`, `mk_diff_snapshots`, `mk_review_change`, `mk_export_debate`, `mk_list_topics`, `mk_get_debate_by_topic`, `mk_list_changes`, `mk_update_iteration`, etc.)
- **`tool-executor.js`**: switch cases for all 25 tools
