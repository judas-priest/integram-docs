...
| `ncl_list_runs` | LOW | List verification runs with pagination |
| `ncl_get_run` | LOW | Get full run details: effective spec, compilation IR, attempts, obligations, evidence, decisions, verification runs |
| `ncl_compare_runs` | LOW | Compare two runs across the 7 canon-12 §10.4 slices |
| `ncl_list_intents` | LOW | List VerificationIntents with optional filters |
| `ncl_create_requirement_version` | MEDIUM | Create a new version of an existing requirement (by stable_id); next version number computed automatically |
| `ncl_list_source_definitions` | LOW | List Source Plane definitions — registered data sources and their snapshot modes |
| `ncl_get_source_definition` | LOW | Get a single Source Plane definition by ID |
| `ncl_list_baselines` | LOW | List requirement baselines — frozen snapshots of active requirements |
| `ncl_get_baseline` | LOW | Get baseline details: included requirements, intents, specs, edges and their counts |
| `ncl_publish_baseline` | HIGH | Publish a new baseline — freezes the current active requirements, intents, specs, and edges |
| `ncl_list_families` | LOW | List artifact families — lineage chains of runs that produced accepted artifacts |
| `ncl_get_family` | LOW | Get family details: all runs, accepted artifacts, schema versions, comparison policy |
| `ncl_list_grants` | LOW | List authority grants — who has which nightcall permissions |
| `ncl_grant_authority` | HIGH | Grant a nightcall authority to a user (optional scope and expiry) |
| `ncl_list_waivers` | LOW | List authority waivers — obligation-level exceptions from verification |
| `ncl_list_edges` | LOW | List active edges of the requirement graph with optional filters (from/to type+id, rel_type) |
| `ncl_create_proposal` | MEDIUM | Create a generic ModelChangeProposal (kind: model_change, handler_mapping, artifact_structure); nothing enters the CNM until accepted via `ncl_decide_proposal` |
| `ncl_create_authority_waiver` | HIGH | Create an authority waiver for a specific obligation — obligation-level exception through the authority system, distinct from `ncl_waive` |
| `ncl_list_declassifications` | LOW | List declassification records — classification level reductions for run artifacts |
| `ncl_create_declassification` | HIGH | Declassify a run's artifacts — lower the classification level (public, internal, confidential, restricted) |
| `ncl_list_outbox` | LOW | List transactional outbox events — pending and published domain events |
| `ncl_drain_outbox` | MEDIUM | Drain the transactional outbox — publish all pending events; returns published/failed/skipped counts |
| `ncl_render_artifact_view` | MEDIUM | Render an accepted run's artifact into a block document (ACCEPTED verdict only) |
...
- `GET /runs`, `GET /runs/:id`, `POST /runs/:id/view` — a run is started by `POST /run` (singular), there is no `POST /runs`
...
- `GET /authority/declassifications`, `POST /authority/declassifications`
...
- **Requirement**: normative statement with modality, subject/predicate/object, status lifecycle (`draft → candidate → [needs_clarification] → proposed → accepted → active → contested/superseded/retired`; `rejected` is a tenth, terminal status reachable from `draft`, `candidate`, `needs_clarification`, `proposed` and `accepted` — never from `active` or later). Statuses beyond candidate require a GovernanceDecision — on both create and update paths.
- **ModelChangeProposal**: the ONLY path from a source into the CNM. `ncl_formalize` stores the Extraction IR here; `ncl_decide_proposal` publishes it (statuses: proposed, deciding, accepted, rejected, needs_clarification; `apply_failed` is legacy-only — since the whole decision became a single transaction it is no longer produced, and a mid-apply failure rolls back to the original status, leaving the proposal retryable). Unresolvable inter-requirement edges are surfaced as `skippedEdges`, never silently dropped.
...
| Operation | Role |
|---|---|
| Reads (`GET`) | any member — except `GET /authority/grants`, which is `requireRole('admin')` |
| Writes, `POST /resolve`, `POST /run` | `editor` |
| `POST /proposals/:id/decide`, `POST /decisions`, `POST /baselines`, `POST /authority/grants`, `POST /authority/waivers`, `POST /authority/declassifications` | `admin` |
| `GOVERNANCE_TOOLS` (`ai/agent/module-gate.js`): `ncl_decide_proposal`, `ncl_governance_decision`, `ncl_waive`, `ncl_publish_baseline`, `ncl_grant_authority`, `ncl_create_authority_waiver`, `ncl_create_declassification` | `admin` |
...
Registry of verification methods. Each method implements an `execute(artifact, plan, ctx)`
function returning an EvidenceEnvelope and an `explain()` function returning a human-readable
description. Five deterministic verifiers are ported as `execute` functions. Workspace methods
are stored in `_v2_ncl_methods`; `resolveMethod(intentKind, evidenceContract, workspaceMethods)`
picks the method during compilation by intent kind **and** the obligation's evidence contract
(`payloadType`) — built-in methods are checked first, then workspace methods, with
`semantic.adequacy` as the catch-all.

## Unit States (`unit-states.js`)

Persists the current per-unit state during engine execution. `setUnitState(pool, db, runId,
unitId, state, reason)` writes to `_v2_ncl_unit_states` (`reason` is a string, optional);
`getUnitStates(pool, db, runId)` returns a `Map` of the CURRENT state per unit — the table
keeps one row per `(run_id, unit_id)` via `ON CONFLICT DO UPDATE`, so no history is retained.

Ten states (canon 12 §6.4): `BLOCKED`, `READY`, `RUNNING`, `CANDIDATE`, `VERIFIED`,
`ACCEPTED`, `FAILED`, `RETRY_REQUIRED`, `HUMAN_REVIEW`, `STALE`. The main chain is
`BLOCKED → READY → RUNNING → CANDIDATE → VERIFIED → ACCEPTED`; `CANDIDATE` also exits to
`FAILED`, `RETRY_REQUIRED` or `HUMAN_REVIEW`, and any state can go `STALE` when a dependency
changes. `ACCEPTED` cannot be rolled back — its only exit is `STALE`. Transitions are validated
against `UNIT_TRANSITIONS`: an illegal one logs a warning and writes nothing. In practice the
engine writes `BLOCKED` (gate), `RUNNING` (generation started), `CANDIDATE` (unit generated),
then `VERIFIED` / `FAILED` / `CANDIDATE` per the evidence relation
