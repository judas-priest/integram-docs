...
backend/src/api/v2/modules/pm/
  schema.js         — DDL для 14 satellite-таблиц, lazy-init
...
frontend/src/
  ...
  views/pm/
    PmLayout.vue              — Layout с табами (Доска, Список, Бэклог, Спринт, Гант, Календарь, Этапы, Метрики, Дорожная карта)
...
14 satellite-таблиц, создаются lazy при первом обращении к PM в воркспейсе:
...
| `_v2_pm_statuses` | Статусы доски (name, label, kind, board_id; `UNIQUE(board_id, name)`) |
| `_v2_pm_milestones` | Этапы (name, description, due_date, status planned/completed, completed_at) |
| `_v2_pm_settings` | Настройки области уровня одной строки (id=1): roadmap_doc_id — документ дорожной карты |
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/board` | Задачи по статусам; ответ `{ columns, totals }` — первая страница каждой колонки (`limit`, умолчание 30, 5–100) и точные totals из окна `COUNT(*) OVER (PARTITION BY status)` |
| GET | `/board/column` | Догрузка одной колонки: те же фильтры + обязательный `status`, `offset`, `limit` (5–100); возвращает `items`, `hasMore`, `total` |
...
72 инструментов, доступны через `search_tools("pm")`. Покрывают HTTP API почти целиком; тулов-близнецов нет у `GET /board/column` (догрузка колонки — клиентская механика) и маршрутов `/roadmap-doc`.

Запрос по имени группы отдаёт все 72: имя группы освобождено от потолка выдачи
...
| `pm_add_checklist_item` | MEDIUM | Добавить пункт в чек-лист (ответ несёт id пункта) |
| `pm_remove_checklist_item` | MEDIUM | Удалить пункт чек-листа по id |
...
| `pm_list_issues_by_target` | LOW | Задачи, привязанные к цели (обратный поиск по таблице/записи/документу/отчёту) |
...
| `pm_get_cfd` | LOW | Cumulative Flow Diagram |
...
| `pm_create_board` | MEDIUM | Создать доску |
| `pm_update_board` | MEDIUM | Переименовать / сменить sort |
| `pm_delete_board` | HIGH | Удалить доску (с переносом задач) |
| `pm_list_statuses` | LOW | Статусы области с kind; опциональный `board_id` — только одной доски |
| `pm_create_status` | MEDIUM | Добавить статус на доску |
| `pm_update_status` | MEDIUM | Переименовать / сменить sort / сменить kind |
| `pm_delete_status` | HIGH | Удалить статус |
...
| Инструмент | Tier | Описание |
|-----------|------|----------|
| `pm_list_milestones` | LOW | Список этапов |
| `pm_create_milestone` | MEDIUM | Создать этап |
| `pm_update_milestone` | MEDIUM | Обновить этап |
| `pm_complete_milestone` | HIGH | Завершить этап |
| `pm_delete_milestone` | HIGH | Удалить этап |
| `pm_get_roadmap` | LOW | Дорожная карта по этапам |
...
Этапы — крупные вехи без дат жизненного цикла спринта: задача привязывается к этапу полем `milestone_id`, спринт и этап независимы. Хранение — `_v2_pm_milestones` (schema.js:48): `name`, `description`, `due_date`, `status` (`planned`/`completed`), `completed_at`.

Маршруты (router.js:448-484):

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/milestones` | Список этапов |
| POST | `/milestones` | Создать этап |
| PATCH | `/milestones/:id` | Обновить |
| POST | `/milestones/:id/complete` | Завершить |
| DELETE | `/milestones/:id` | Удалить |
| GET | `/milestones/:id/issues` | Задачи этапа |

AI-тулы: `pm_list_milestones` (LOW), `pm_create_milestone` (MEDIUM), `pm_update_milestone` (MEDIUM), `pm_complete_milestone` (HIGH), `pm_delete_milestone` (HIGH).

---

## Roadmap («Дорожная карта»)

Дорожная карта строится по этапам и их срокам (`GET /roadmap`, router.js:488-492). К карте можно привязать документ: id хранится в единственной строке настроек области `_v2_pm_settings` (schema.js:168, колонка `roadmap_doc_id`).

Маршруты (router.js:655-671):

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/roadmap` | Данные дорожной карты |
| GET | `/roadmap-doc` | Привязанный документ |
| POST | `/roadmap-doc/create` | Создать документ и привязать |
| POST | `/roadmap-doc` | Привязать документ (`{ docId }`) |

AI-тул: `pm_get_roadmap` (LOW).
...
DataTable (после PM-294): фильтры колонок, сортировка по клику, теги вместо голых значений, правка по клику, клиентская пагинация.

- **Тулбар**: поиск, фильтр по статусу (серверный), фильтр по меткам, область исполнителя (Мои/Все/участник), сброс всех фильтров одной кнопкой.
- **Фильтры колонок**: у «Приоритета» — клиентский фильтр встроенной механикой DataTable; статус и исполнитель в колонках не дублируются — теми же значениями правит тулбар и область PM-293.
- **Сортировка по клику** во всех колонках; по кодам нельзя — «high» < «low» алфавитом — поэтому приоритет сортируется по рангу, статус по метке, исполнитель по имени.
- **Правка по клику**: статус и приоритет в покое выглядят тегами (цвет статуса — по машинному `kind`, не по литералам кодов); клик по ячейке меняет её на Select, выбор сохраняется мгновенно.
- **Пагинация на клиенте**: 25/50/100 строк, умолчание 25.
- **Подзадачи**: раскрываются шевроном, дети догружаются с серверными фильтрами экрана; подзадача, отфильтрованная по статусу/поиску, исчезает вместе с родителем.
...
Collapsible секция "ЗАДАЧИ" с подпунктами: Доска, Список, Бэклог, Спринт, Гант, Календарь, Этапы, Метрики, Дорожная карта. Каждый пункт — `router-link` с `?tab=` query параметром.
...
| Клавиша | Действие |
|---------|----------|
| `Ctrl/Cmd+C` | Открыть форму создания задачи (с выделенным текстом не перехватывается — копирование) |
| `Alt+1`-`Alt+6` | Переключить вкладку (Доска, Список, Бэклог, Спринт, Гант, Календарь) |

Горячих клавиш нет у трёх последних вкладок — «Этапы», «Метрики», «Дорожная карта»; только сайдбар и TabMenu.
...
| Файл | Строк | Описание |
|------|-------|----------|
| `schema.js` | ~370 | DDL 14 таблиц + индексы |
| `service.js` | ~1197 | Issues CRUD, hierarchy, history, каскад мягкого удаления, bulk в транзакции, чек-лист, перенос между досками |
...
| `board.js` | ~226 | Board/backlog aggregation + догрузка колонки (getBoardColumn) |
...
72 tool handler функций, TOOL_DEFS в `agent/index.js`, cases в `tool-executor.js`, тиры в `risk-tiers.js`. HTTP API покрыт почти целиком; исключения — `GET /board/column` и маршруты `/roadmap-doc` (тулов-близнецов нет).
...
- **AI**: `pm_toggle_checklist` отмечает пункт по `item_id` (без `done` — переворачивает нынешнее состояние). `pm_update_issue` поле `checklist` не пробрасывает, а `PATCH /issues/:id` и `PATCH /issues/bulk` отвергают его с 400; добавление и удаление пункта — `pm_add_checklist_item` (текст обязателен, ответ несёт `id` пункта) и `pm_remove_checklist_item` (по `item_id`), обе MEDIUM
