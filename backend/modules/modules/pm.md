...
66 инструментов, доступны через `search_tools("pm")`. Полное покрытие HTTP API.

Запрос по имени группы отдаёт все 66: имя группы освобождено от потолка выдачи
...
66 tool handler функций, TOOL_DEFS в `agent/index.js`, cases в `tool-executor.js`, тиры в `risk-tiers.js`. Полное покрытие HTTP API модуля.
