...
**Files:** `router.js`, `service.js`, `auth.js`, `api.js`, `private-api.js`, `write-api.js`, `analytics.js`, `cart.js`, `guest-order.js`, `legal.js`, `config-utils.js`, `roles.js`, `a2a.js`, `agent.js`, `chat/`, `teamchat/`, `staff-cdek.js`, `staff-dadata.js`, `staff-orders.js`, `account-service.js`, `agent-registry-proxy.js`, `code-guard.js`, `portal-agent-ctx.js`, `row-rules.js`, `telegram-broadcast.js`, `telegram-callback.js`, `telegram-webhook-secret.js`, `thread-id.js`, `uds-webhook.js`, `meta-kb-proxy.js`, `decisions-proxy.js`
...
|--------|------|-------------|
| POST | `/portal/api/auth/request` | Request OTP via SMS or email. Body: `{ phone }` or `{ email }` (email needs `auth.emailReqId`) |
| POST | `/portal/api/auth/verify` | Verify OTP, set `portal_jwt` httpOnly cookie. Body: `{ phone, code }` or `{ email, code }` |
...
`GET /portal/api/auth/me` verifies that `clientObjectId` from the token still points to an existing object (TD-083). If the object was deleted: `clientObjectId` is `null` and `clientUnlinked: true` ("there WAS a link and it broke" — distinct from "never linked"); for customers, phone/name/role/pages are not queried at all. `POST /portal/api/objects` responds `409 { error: { code: 'CLIENT_UNLINKED' } }` for sessions with a dead link. Staff login resolves the client link inside `auth.clientsTypeId` (and column `auth.emailReqId` when set); without the config keys the legacy wide search stays, with a warn. The staff branch additionally resolves `name` from `_v2_users` by userId (fail-soft — a failed query just hides the name; `router.js`, commit `9c13cb783`) so chat identity has a human name instead of an email fallback.
...
Four layers protect the `/portal/telegram/:secret` endpoint:
...
3. **Message_id deduplication** (`telegram-dedup.js#markMessageSeen`) — a second
   dedup level for `message` updates: Redis key
   `tgmsg:<botSecret>:<chat_id>:<message_id>` with 24h TTL via `SET NX`,
   fail-open if Redis is down. Guards against a re-delivery that carries a NEW
   `update_id` but the same message (duplicate intake dispatch). Gates
   message-updates before the command handlers and the intake dispatcher
   (`portal/router.js`); `edited_message` carries its own update key and is
   not swallowed by this guard.

4. **Outbound rate limiting** (`telegram-rate-limit.js`) — `telegramSendMessage`
...
| POST | `/portal/api/upload` | File upload from custom_code (portal JWT, 20 req/min, max 500 MB, streamed to disk). Returns `{ id, filename, url }` |
...
| GET | `/portal/api/documents-list` | Block documents visible to the staff member (grants-filtered; lazy `ensureSideTables`) |
| POST | `/portal/api/documents` | Create a block document |
| PATCH | `/portal/api/documents/:docId` | Rename a document (title only) |
| POST | `/portal/api/documents/:docId/blocks` | Append a block |
| PATCH | `/portal/api/documents/:docId/blocks/:blockId` | Patch a block |
| DELETE | `/portal/api/documents/:docId/blocks/:blockId` | Delete a block |

All six document routes carry `requireStaffPortalJwt` + `portalGrantsMiddleware` and run
`ensureSideTables` lazily, so a fresh workspace without the document tables gets them created
instead of a 500.

### Other endpoints missing from the tables above

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/portal/api/kag/search` | portal JWT, 30/min | Knowledge-graph search for portal pages |
| GET | `/portal/api/upload/:fileId/text` | portal JWT | Extracted file text (poll until ready) |
| GET/PUT/DELETE | `/portal/api/account/llm-key` | portal JWT | Per-client personal LLM key (settings) |
| GET | `/portal/api/agent/conversations` | portal JWT | User's agent conversations |
| GET | `/portal/api/agent/conversations/:id/messages` | portal JWT | Conversation history (last 100) |
| POST | `/portal/api/agent/conversations/:id/share` | portal JWT | Create public share link for a conversation |
| DELETE | `/portal/api/agent/conversations/:id/share` | portal JWT | Revoke the share link |
| POST | `/portal/api/agent/topics/:id/share` | staff JWT | Create public share link for a teamchat topic |
| GET | `/portal/api/shared/chat/:token` | none | Public: load a shared conversation by token |
| GET | `/portal/api/code/tree`, GET/POST `/portal/api/code/file` | staff JWT | Browse/read/write custom-code repo files |
| GET | `/portal/api/generated-files/*` | staff JWT | Serve generated files from custom code |
| POST | `/portal/api/tools/run`, `/portal/api/code/run` | staff JWT | Run allow-listed workspace tools / repo code |
...
`tools.allowDelegation` (по умолчанию `true`) — разрешает staff с `allowWrite`
делегировать задачи встроенным под-агентам без ограничений слагов. `false` —
делегация для роли живёт в белом списке `allowedAgentSlugs`, запись выполняет
сам оркестратор.
...
- `set_portal_config(merge: true)` deep-merges objects (RFC 7386: a `null` value
  deletes the key), but **arrays are replaced wholesale** — a partial `pages[]`
  wipes every page. For a one-module change use `update_portal_module` instead.
- `update_portal_module(slug, config, moduleIndex?)` deep-merges into
  `modules[moduleIndex ?? 0].config`: nested objects (e.g. `bindings`) merge by
  key; a `null` value deletes the key; **arrays and scalars are replaced
  wholesale**; keys you omit are preserved. `moduleIndex` is positional; the
  first module is the default.
