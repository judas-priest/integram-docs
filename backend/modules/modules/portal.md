...
`GET /portal/api/auth/me` verifies that `clientObjectId` from the token still points to an existing object (TD-083). If the object was deleted: `clientObjectId` is `null` and `clientUnlinked: true` ("there WAS a link and it broke" — distinct from "never linked"); for customers, phone/name/role/pages are not queried at all. `POST /portal/api/objects` responds `409 { error: { code: 'CLIENT_UNLINKED' } }` for sessions with a dead link. Staff login resolves the client link inside `auth.clientsTypeId` (and column `auth.emailReqId` when set); without the config keys the legacy wide search stays, with a warn.
