...
**Auth:** JWT required. POST: editor+ (плюс CSRF), GET: viewer+.
...
тот же судья, что у канала `objects`; для `record` — он же по типу записи плюс
построчные правила `checkRowPermission(..., 'READ', ...)`. Отказ:
...
молча выходит, **не** отвечая `NOT_SUBSCRIBED` (в отличие от каналов `objects`,
`documents`, `sandbox-collab`).
...
«relation does not exist» (замер 20.08.2026: таблица была в 11 схемах примерно из ста).
...
20.08.2026: таблица была в 4 схемах из 206 воркспейсов. Досыпка по всем схемам —
