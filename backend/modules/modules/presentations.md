...
| POST | `/bindings/preview` | editor | Резолв текста с плейсхолдерами (тело `{ text }` → `{ resolved }`) — для редактора |
...
| `modules/presentations/binding.js` | биндинги `{{report:…}}`, `{{obj:…}}`, `{{today}}`: парсер, резолв, форматтер |
| `modules/presentations/engine/to-pptx.js` | модель → .pptx (резолв биндингов при ctx) |
| `modules/presentations/engine/to-html.js` | модель → HTML/PDF (общий Chromium, резолв биндингов при ctx) |
| `modules/presentations/engine/from-pptx.js` | .pptx → модель + warnings |

## Биндинги

Плейсхолдеры хранятся в модели, резолвятся в точках рендера
(`modelToPptxBuffer(slides, ctx)`, `modelToPdfBuffer(slides, ctx)`). Кэша рендера
нет; если появится — резолв обязан быть после кэша. Отсутствующее значение →
видимый `⚠` перед плейсхолдером.
