...
- `func`: `SUM` | `AVG` | `COUNT` | `MIN` | `MAX` | `GROUP_CONCAT` | `STRING_AGG`. System functions: `ABN_ID`, `ABN_UP`, `ABN_TYP`, `ABN_ORD`, `ABN_REQ`, `ABN_BT`, `ABN_URL`, `ABN_ROWNUM`, `ABN_DOMAIN`, `ABN_TAG`. Transform functions: `DATE_TRUNC`, `UPPER`, `LOWER`, `LENGTH`, `TRIM`, `ROUND`, `EXTRACT`, `COALESCE`, `ABS`, `CEIL`, `FLOOR`. `PATCH` с `func: null` снимает агрегат (удаляет подсвойство).
...
- `sortDir`: column sort direction — `ASC` or `DESC`. Sorting by a computed (ROLLUP/FORMULA/LOOKUP) column sorts post-query in JS over a capped row set (5000; `logger.warn` at the cap) — the page is sliced after sorting.
...
- Tokens in date fields and stored WHERE: `[TODAY]`, `[NOW]`, `[MONTH_START]`, `[MONTH_END]`, `[USER]`, `[USER_ID]`, `[ROLE]`, `[ROLE_ID]`. Date tokens use the client timezone (`x-tzone` header, minutes from UTC). Tokens are substituted as query parameters, never as inline text. Request tokens (`[REMOTE_ADDR]`, `[HTTP_*]`) are not supported.
...
```json
{
  ...
  "select": "1,2,3",
  "fieldNames": "Имя,Сумма",
  "filterId": 42
}
```

- `select`: comma-separated string of column IDs to include in the result (subset of report columns)
- `fieldNames`: comma-separated list of column names to include in the result (subset of output columns; not a flag)
- `filterId`: apply a saved filter by ID
- Filter operators: `from`/`to` (range), `eq` (exact match), `like` (pattern match with `%` wildcards)
- Sorting: numeric columns (NUMBER/SIGNED/CURRENCY/PERCENT/DURATION/RATING/AUTO_NUMBER) sort numerically in non-aggregate reports; in aggregate reports they (like dates) fall back to textual sorting — an output alias in ORDER BY cannot be wrapped in an expression
...
- `a.id`, `a.val`, `a.up`, `a.ord` are wrapped in `MIN()` in SELECT — `a.id` must NOT appear as a bare expression in GROUP BY, because it is unique per row and would prevent any actual aggregation.
- Only non-aggregate, non-hidden columns go into GROUP BY; the main column enters as `a.val` (it is selected bare, so it must be a group key), while an aggregated main column (its own SUM/COUNT func) does not.
