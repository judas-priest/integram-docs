...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/teamchat/topics/:topicId/upload` | Upload file and create message with file card (multipart, max 500 MB, streamed to disk) |
...
2. **Engineer** (`sandbox-engineer`) — generates code cells reviewed in the cycle. Note: `send_teamchat_message` IS a global TOOL_DEFS entry (group `teamchat`), but the sandbox-engineer prompt explicitly forbids calling it — code cells reach the topic through the cycle, not direct tool calls
...
### Gap-Closing Tools (`ai/agent/tools/teamchat-gaps.js`)

13 tools closing REST parity gaps (all in group `teamchat`, global TOOL_DEFS):

| Tool | Risk Tier | Description |
|------|-----------|-------------|
| `list_public_rooms` | TIER_LOW | List public rooms joinable without membership |
| `forward_message` | TIER_MEDIUM | Forward/copy a message to another topic |
| `send_topic_file` | TIER_MEDIUM | Post a file-card message by fileId (files judge `requireReadGrant`) |
| `refresh_topic_document` | TIER_MEDIUM | Regenerate the topic document from topic messages |
| `create_reminder` | TIER_MEDIUM | Create a reminder on a topic |
| `list_reminders` | TIER_LOW | List reminders |
| `delete_reminder` | TIER_MEDIUM | Delete a reminder |
| `get_message_reactions` | TIER_LOW | Get emoji reactions on a message |
| `add_message_reaction` | TIER_MEDIUM | Add an emoji reaction to a message |
| `remove_message_reaction` | TIER_MEDIUM | Remove an emoji reaction |
| `request_topic_approval` | TIER_MEDIUM | Ask admins for topic approval (human resolves via notifications) |
| `list_topic_tasks` | TIER_LOW | Task-flagged messages of a topic (filter `list_recent_topics` lacks) |
| `update_room_member_role` | TIER_MEDIUM | Change a room member role |
