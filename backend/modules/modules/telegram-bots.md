...
See [portal.md](portal.md#telegram-webhook-hardening) — four layers protect the shared `/portal/telegram/:secret` endpoint:
1. Header verification (`X-Telegram-Bot-Api-Secret-Token`)
2. Update_id deduplication (Redis, 24h TTL)
3. Message_id deduplication (`tgmsg:<botSecret>:<chat_id>:<message_id>`, Redis, 24h TTL) — message updates only
4. Outbound rate limiting (30/sec global, 1/sec per chat)
