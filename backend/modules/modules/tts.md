...
- `wavFromPcm(pcm, sampleRate)` — строит WAV-контейнер над raw PCM Piper; единый экземпляр для REST `/synthesize` и автоматизации `send_telegram_media` (раньше сборка жила инлайном в роутере)
