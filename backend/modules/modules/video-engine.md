...
- `POST /jobs/:jobId/publish` — опубликовать готовый mp4 в файлы воркспейса (editor): копия из
  `UPLOAD_DIR/<db>/video-engine/<jobId>/` в корень файлов + строка в `_v2_files`. Идемпотентно:
  опубликованный и живой на диске файл возвращается как есть (`alreadyPublished: true`), удалённый —
  публикуется заново. Ответ: `{ ok, published: { filename, fileId, size, publishedAt }, alreadyPublished }`.
  Ролик становится виден в «Файлах», качается через `GET /files/<filename>` и раздаётся порталу
  через `GET /:db/portal/api/files/<filename>` (mp4 требует портальную сессию — `portal/router.js` MEDIA_EXTS).
...
Страница `/:db/video` «Медиа-студия» (`frontend/src/views/video/VideoPage.vue`, маршрут `video`): список джоб с прогрессом по стадиям, предпросмотр сториборда, встроенный плеер (blob + revoke), отмена, публикация готового ролика в Файлы (кнопка «Опубликовать в Файлы», чип опубликованного файла; запись в навигации сайдбара — `modules.video = true`). Сервис — `frontend/src/services/video.js`.
...
| `vid_publish_job` | MEDIUM | Опубликовать готовый mp4 джобы в файлы воркспейса (идемпотентно) |
