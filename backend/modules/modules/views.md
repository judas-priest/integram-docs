...
Shared views enforce the same column visibility and filters as the saved config. Saved filters are stored in screen form (`EQ_<colId>`, `LIKE_<colId>`, `IS_NULL_<colId>`, ... — see `FILTER_OP_MAP` in `ObjectList.vue`) and are normalized to the `req_<colId>` DSL by `normalizeViewFilters` in `service.js` before the public path applies them. An unsupported operator in a saved filter rejects the request with 400 (`VALIDATION_ERROR`) instead of silently widening the result set. Legacy `req_<colId>` keys and array-shaped `filters` are still tolerated.
