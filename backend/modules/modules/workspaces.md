...
| POST | `/workspaces` | Create workspace (`name`, `slug`, optional `template`/`templateId`, `org_id`, `seedData`, `entityLocalIds`, `entityOverrides` — `[{ localId, name }]`, renames a template entity on apply, `sharedKag`) |
| GET | `/workspaces/:slug` | Get workspace details and settings |
| PUT | `/workspaces/:slug` | Update name, settings (modules, security, ai, agent, icon, description, credentialsTypeId), `is_template`, `slug` (owner only — changes the public URL; the old slug is kept in `_v2_slug_redirects` and served as `307 → current slug` until reclaimed) |
...
| POST | `/workspaces/:slug/leave` | Выйти из воркспейса (owner и последний админ — 403) |
...
| POST | `/workspaces/:slug/invitations/link` | Create one-time invite link — see Invite links below |
| DELETE | `/workspaces/:slug/invitations/:id` | Revoke invitation (works for link-based invitations too) |

#### Invite links

`POST /workspaces/:slug/invitations/link` — одноразовая пригласительная ссылка
вместо письма (`workspace-invitations.js`, `createInviteLink`). Тело:
`{ role, email?, meta?, expiresInDays? ≤ 30 }`; срок по умолчанию — 7 дней.
Ответ `201 { ok, data: { id, token, url, role, email, meta, expiresAt } }`, где
`url = ${APP_BASE_URL}/invite/<token>` — страница активации фронтенда.

Отличия от email-приглашения:

- **Приглашать может любой участник воркспейса**, не только админ (решение
  владельца, 23.09.2026). Выдаваемая роль капится уровнем приглашающего:
  editor не выдаёт admin; **`owner` ссылкой не выдаётся никогда** — путь один,
  `transferOwnership`. Фактически выданная роль возвращается в ответе как `role`.
- **`email` необязателен**: авторизация при активации — сам токен (токен 32
  случайных байта, `base64url`, хранится в `_v2_invitations.token` plaintext).
  С адресом — проверка «уже член воркспейса» на создании (409), как у письма.
- `meta` — непрозрачный JSON-payload воркспейса (например, контекст бота), в
  триггер `member.joined` он уходит как есть.

`GET /api/v2/invites/:token/peek` — публичный (без авторизации; секрет — сам
токен) контекст ссылки для страницы активации: `{ workspace: { name, slug },
inviter, role, email: string|null, meta, expiresAt }`. Погашенный (принят /
отозван) или истёкший токен — 404; peek токен не потребляет.

Принятие — существующий `POST /api/v2/invites/:token/accept` (JWT): `acceptInvitation`
теперь принимает и ссылки без email — проверка почты выполняется только когда
приглашение создано с адресом. Успех создаёт членство и эмитит `member.joined`
(контекст триггера автоматизаций — в `automations.md`).
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/workspaces/wizard/analyze` | Analyze free-text business description → match template + extract entities/modules. Body: `{ description, teamSize?, hasClients? }`. Returns `{ templateId, templateSlug, templateName, entities, modules, confidence, normalizerHints, kagSourceWorkspace }` — the last two are copied straight from the matched template's manifest and are `null` when it declares neither. The "no templates at all" answer is one field short: it carries `normalizerHints: null` and **no `kagSourceWorkspace` key at all** — an absent field, not an empty one. |
...
| POST | `/workspaces/copy-table` | Copy one table (with its lookups, ref targets and child tables) from one workspace into another — see Fragment Copy below |
...
Copies a SINGLE table from workspace A to workspace B together with its transitive closure: lookup tables, ref targets and child tables, plus the parent chain for child tables (a child cannot exist without its parent — apply would otherwise create a broken type row) (BFS from the root; each table's role — `root`/`child`/`parent`/`ref` — is named in the response). Built on the template machinery (ADR-029), not a separate channel. Body: `{ sourceSlug, targetSlug, table (name or typeId), dryRun=false, includeData=false }`. Admin role required on BOTH workspaces — checked separately, a foreign slug is a 403, not silence. Copying onto itself (`sourceSlug === targetSlug`) is refused with 400 — name-based adoption would turn it into a silent in-place update, not a copy. Same code is exposed as the AI tool `copy_table_to_workspace` (TIER_HIGH, HITL confirmation).

Mechanics (`workspace-table-copy.js`):

- **Manifest pruning is load-bearing.** The source manifest is extracted (`extractWorkspaceManifest`) and cut down to the closure types (`types` + `demoData`). BlockPick cannot express this: unclaimed types travel as "backbone" (template-blocks.js: "незаявленное — основа"), so a block selection would drag the whole source workspace. A live `manifest.portal` must not survive pruning either — apply writes portal config `ON CONFLICT (db) DO UPDATE` and would overwrite the target's portal. Views, reports, computed columns, automations, documents, dashboards, connectors, roles and portal are NOT carried; the response lists them in `stripped`.
- **Idempotent re-run.** The fragment is stored as a PRIVATE template (visible only to its owner) with the deterministic slug `copy-<typeId>-from-<sourceSlug>`. Re-running the same copy UPDATEs the template and re-applies under the same origin-map key → update in place, no duplicates. Note: `dryRun` still creates/updates this private template row — it is the idempotency anchor; the apply plan itself is produced without changes.
- **Apply** is the standard `applyTemplateToWorkspace` (ID remapping, `dryRun` plan, per-section report). `includeData=true` carries records; the template ceiling is 5000 records counted over ALL source-workspace records, so a large source is refused with 400 rather than truncated.
- **localId guard.** Real type ids are mapped to manifest localIds (`T<n>`) by mirroring the extractor's ordering (`ORDER BY ord, id`); every computed localId must exist in the extracted manifest, else the copy is refused (INTERNAL_ERROR) instead of silently dropping types.

Response: `{ templateId, templateSlug, rootTypeId, rootName, fragment: { typeIds, roles }, fragmentManifest, stripped, apply }` where `apply` is the dryRun plan or the ADR-029 per-section report.
...
Templates are stored as JSON manifests capturing the workspace schema (types, reqs, views, reports, seed data). On `createWorkspace` with `templateId`, the manifest is replayed with EAV ID remapping. `applyTemplateToWorkspace` supports `dryRun` mode.

> **Row rules do not travel in a template.** The carry registry declares
> `_v2_row_rules` as `template: { mode: 'skip' }`
> (`registry/workspace-carry.js`) — and neither path reads them, not
> `createWorkspaceFromTemplate` and not `applyTemplateToWorkspace`. The
> consequence is a widening of access, not a narrowing: a workspace unrolled
> from a template gets the roles **without the row restrictions** that limited
> them in the source. Row rules are carried by the *clone* path only, and there
> by a dedicated function (`clone.mode: 'custom'`, `cloneRowRules`).
...
source (`carryColumns` in `src/api/v2/registry/carry-catalog.js`, used by
`copyTable` in `workspace-carrier.js`). Three guards keep the registry
...
`workspace-carrier.js` holds the per-table carry mechanics (`copyTable`,
`copyScoped`, `stripCarried`, `renewInValue`, `carryPlan`, `insertMapped`,
`loadRows`, `rowFailures`). The questions asked of the DB catalog live one level
up, in `src/api/v2/registry/carry-catalog.js` (`carryCatalog`, `carryColumns`,
`isOwnNumber`, `primaryKeyColumns`, `safeColumn`, `blankMap`, `advanceIdentity`):
clone is not their only reader — backup/restore (`modules/admin`) and the APM
snapshot (`middleware/apm-diag.js`) ask the same questions, and a second reader
would drift from the first. The phase functions named
...
`OVERRIDING SYSTEM VALUE` (`cloneWorkspace`, `workspace-clone.js`), and satellite rows keep
their own ids the same way (`copyTable`, `workspace-carrier.js`): satellites reference each
...
Order in `deleteWorkspace` (`service.js`): `DROP SCHEMA` on the workspace's own
...
DDL lives in the `DDL` array of `src/api/v2/registry/tables.js`, later columns in
its `MIGRATIONS` list (except `_v2_service_keys`, see below). Referenced by symbol
on purpose — line numbers rot.

**`_v2_workspaces`** — `id`, `slug`, `name`, `db_name` (`VARCHAR(15)`),
`owner_id`, `template`, `is_template` (bool), `settings` (JSONB), `remote_dsn`
(JSONB, nullable), `created_at`. Unique on `slug` and on `db_name`. Привязка к
организациям — через `_v2_org_workspaces` (см. `orgs.md`).
...
**The table-level `UNIQUE (user_id, org_id, workspace_id)` catches nothing:**
both ownership columns are nullable and two NULLs are never equal in Postgres.
Real uniqueness comes from two partial indexes declared by `MEMBERSHIP_UNIQUE`
(`registry/tables.js`): `uq_membership_workspace` on
`(user_id, workspace_id) WHERE workspace_id IS NOT NULL` and `uq_membership_org`
on `(user_id, org_id) WHERE org_id IS NOT NULL`. `ON CONFLICT` must target one
of those pairs — and must repeat the predicate — never the triple.

**`_v2_invitations`** — `email`, `inviter_id`, `org_id`, `workspace_id`, `role`
(no `owner`), `token` (unique), `status` (`pending`/`accepted`/`expired`/`revoked`),
`expires_at`, `created_at`. Accept валиден только из `pending`: UPDATE несёт
`AND status = 'pending'`, иначе 409 — гонка с revoke и двойной accept закрыты.
...
(`CHAR(64)`, **HMAC-SHA256** of the raw token, unique — `createServiceKey` in
`workspace-bots.js` is the only writer, keyed by `SK_HMAC_SECRET`, falling back
to `JWT_SECRET` and then to the literal `'integram-sk'`), `key_prefix` (`VARCHAR(12)`, first
...
> **Bare SHA-256 is a reading fallback, not the storage format.**
> `resolveServiceKey` (`middleware/service-key-auth.js`) computes both digests
> and looks the key up with `WHERE sk.key_hash IN (?, ?)` — HMAC first, bare
> SHA-256 kept only for keys issued before the change. Nothing writes the bare
> form any more. On the reading side an unset `SK_HMAC_SECRET` falls back to
> `JWT_SECRET` with a warning, and with neither set the request fails 500 —
> the writer's third fallback (`'integram-sk'`) has no counterpart here.

> A second, shorter list of the same five tables used to stand here. It was
> removed rather than corrected: it had already fallen behind by two facts
> (`_v2_memberships` was missing `last_seen_at`, `_v2_service_keys` still said
> bare SHA-256), and a second copy of a column list drifts from the first
> silently. Everything it held is above; the `MEMBERSHIP_UNIQUE` note moved into
> the `_v2_memberships` section.
