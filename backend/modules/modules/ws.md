...
- `pointType === 'table'` → `canSubscribeObjects(grants, pointId)` — literally the
  objects-channel gate, including its "no numeric grants at all → allow" legacy
  fallback.
- `pointType === 'record'` → one query resolves the record's type
  (`SELECT t FROM <eav> WHERE id = ?`), then the same `canSubscribeObjects` on that
  type, then `checkRowPermission(..., 'READ', ...)` — the row-level rules, as in
...
| `handlePointOp` | **Returns silently — no error frame at all.** |

The `point` outlier is worth knowing when debugging: a client whose subscribe
frame was rejected, or which unsubscribed and kept sending `intent`, gets no
feedback whatsoever — its ops simply vanish.
