...
Шкала размеров текста для ВСЕГО фронтенда (Tailwind):

| класс | размер | назначение |
|--------|--------|------------|
| `text-xs` (0.75rem) | 12px | вторичный текст, метаданные, теги/чипы — минимум для читаемого текста |
| `text-sm` (0.875rem) | 14px | **базовый размер** интерфейса: контент, таблицы, формы |
| `text-lg` (1.125rem) | 18px | подзаголовки |
| `text-xl` / `text-2xl` | 20 / 24px | заголовки страниц, крупные цифры |

Правила:

1. База любого view — `text-sm`. Произвольные `text-[Npx]` и `font-size` в
   scoped-стилях допускаются ТОЛЬКО из этой шкалы (`0.75rem` / `0.875rem` /
   `1.125rem` / `1.25rem` / `1.5rem`).
2. Размеры вне шкалы (10px, 11px, 13px, 0.85rem…) запрещены — текст 10–11px
   нечитаем, дробные rem плодят зоопарк. Живой кейс: PM до нормализации
   нёс 10/11/13/13.6/16px одновременно.
3. Жирность: `font-semibold` (600) — максимум для выделения; `font-bold` (700)
   не используется.
4. Иконки-бейджи счётчиков (не текст) могут быть меньше — `text-[9px]`
   допустим только у абсолютных бейджей поверх аватаров.
5. Сторож шкалы: `src/views/pm/__tests__/pm-typography-guard.spec.js` —
   проходит по обоим корням PM-раздела (`src/views/pm` и `src/components/pm`);
   при переносе шкалы на всё приложение расширить обход на `src/views`
   и `src/components`.
