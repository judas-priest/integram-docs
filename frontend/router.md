...
| `/access-denied` (и `/workspace-not-found`) | `access-denied` (и `workspace-not-found`) | `auth/WorkspaceNotFound.vue` — нейтральная страница «Рабочая область недоступна» для 403 и 404 DB_NOT_FOUND |
...
| `/invites/:token/accept` | `invite-accept` | `auth/InviteAccept.vue` — **вне `AppLayout`**: у получателя приглашения может не быть ни одной рабочей области, а боковое меню предполагает обратное |
...
| `/:db/reports/:reportId/matrix` | `report-matrix` | `reports/MatrixReportView.vue` | reports |
| `/:db/reports/:reportId/edit` | `report-edit` | `reports/ReportEdit.vue` | reports |
| `/:db/reports/:reportId/builder` | `report-builder` | *(redirect)* → `/:db/reports/:reportId/edit` | reports |
...
| `/:db/pm` | `pm` | `pm/PmLayout.vue` | pm |
| `/:db/nightcall` | `nightcall` | `nightcall/NightcallLayout.vue` | nightcall |
...
| `/:db/admin/dlp` | `admin-dlp` | `admin/DlpRules.vue` | — |
| `/:db/admin/qa-results` | `admin-qa-results` | `admin/QaResults.vue` | — |
| `/:db/telegram-bots` | `tg-bots` | `admin/TgBotList.vue` | — |
| `/:db/telegram-bots/:id` | `tg-bot-edit` | `admin/TgBotEdit.vue` | — |
...
| `/:db/settings/sessions` | `settings-sessions` | `settings/Sessions.vue` | — |
