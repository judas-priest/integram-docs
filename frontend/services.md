...
- При 403 "Workspace access denied" и 404 DB_NOT_FOUND — редирект на нейтральную страницу `/access-denied` (и `/workspace-not-found`) «Рабочая область недоступна», слаг — в query.ws
...
|------|---------------|-------------|
| `iam.js` | iam | `/api/v2/iam/` — профиль: `uploadAvatar(file)`, `updateMe` (сброс аватара `avatarUrl: null`), сессии: `listSessions(offset)`, `revokeSession(id)`, `revokeOtherSessions()`, смена email: `requestEmailChange({currentPassword, newEmail})`, `confirmEmailChange(token)`, `getPendingEmailChange()` |
...
| `decisionsService.js` | decisions | `/api/v2/:db/decisions/` — `list`, `create`, `get`, `update`, `delete`; `listLinks`, `createLink`, `deleteLink`; `getDiscussions`; `getConflicts(db, id)`, `getHistory(db, id)`, `getKagStats(db, id)`; `getIterations(db, id)` → `GET /decisions/:id/iterations`, `getGraph(db)` → `GET /decisions/graph` |
| `metaKbService.js` | meta-kb | `/api/v2/:db/meta-kb/` — `listTopics(db)`, `deleteTopic(db, topicId)`, `getTopicDebate(db, topicId)`, `listTopicDebates(db, topicId)`, `listDecisionDebates(db, decisionId)`, `getDebate(db, debateId)`, `reviewChange(db, id, data)`, `getAnalytics(db)`, `exportDebate(db, debateId)`, `exportDebateDocx(db, debateId)`, `exportMarkdownDocx(db, markdown, filename)`, `listRules(db)`, `createRule(db, data)`, `runRules(db, entityIds)`, `getWelcome(db)`, `listChanges(db, status, limit)`, `proposeChange(db, data)`, `listSnapshots(db, limit)`, `createSnapshot(db, label)`, `diffSnapshots(db, a, b)`, `deleteRule(db, id)`, `listIterations(db, params)`, `updateIteration(db, id, data)` |
...
| `pm.js` | pm | `/api/v2/:db/pm/` — `listIssues`, `getIssue`, `createIssue`, `updateIssue`, `deleteIssue`; `listSprints`, `createSprint`, `updateSprint`, `startSprint`, `completeSprint`; `getBoard`, `getBacklog`; метрики по отдельному методу на каждую: `getVelocity`, `getBurndown`, `getCycleTime`, `getLeadTime`, `getCfd`, `getWorkload` |
