...
- `iterations` — итерации (дебаты) текущего решения
...
- `loadIterations(db, decisionId)` — GET `/decisions/:id/iterations`
...
- `board` — `{ columns, totals, offsets }` — Kanban-доска
- `backlog` — задачи без спринта
- `statuses` — статусы активной доски
- `boards` — список досок
- `milestones` — этапы
- `roadmap` — `{ epics, milestones }` — дорожная карта
- `activeBoardId` — id активной доски
- `childrenByParent` — `Map<parentId, issues>` — подзадачи, догруженные при раскрытии
- `assigneeScope` — область по исполнителю: `'me' | 'all' | '<id>'` (PM-293)
- `loading` — флаг загрузки

**Computed:**
- `activeSprint` — текущий активный спринт
- `planningSprints` — спринты в планировании
- `scopeUserId` — числовой id для сервера (`'me'` → текущий пользователь, PM-293)

**Actions:**
- `loadIssues(db, filters)` — GET `/pm/issues`
- `loadChildren(db, parentId, filters)` — подзадачи одной задачи (`?parentId=`)
- `loadBoard(db, params)` — GET `/pm/board`
- `loadBoardColumn(db, status)` — догрузка хвоста колонки (offset = текущая длина колонки)
- `loadBacklog(db)` — GET `/pm/backlog`
- `loadSprints(db)` — GET `/pm/sprints`
- `loadMilestones(db)` — GET `/pm/milestones`
- `loadRoadmap(db)` — GET `/pm/roadmap`
- `loadStatuses(db)` — статусы активной доски
- `loadBoards(db)` — список досок, восстановление активной из localStorage
- `setActiveBoard(db, id)` — смена доски + перечитывание статусов
- `setAssigneeScope(db, value)` / `restoreAssigneeScope(db)` — область по исполнителю (PM-293)
