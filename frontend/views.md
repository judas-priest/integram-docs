...
| `pm/` | `PmLayout`, `BoardView`, `ListView`, `BacklogView`, `SprintView`, `GanttView`, `CalendarView`, `MetricsView` | Задачи. Вкладки переключаются параметром `?tab=` (`board`, `list`, `backlog`, `sprint`, `gantt`, `calendar`, `metrics`). `MetricsView` — скорость, сгорание, накопительная диаграмма потока, загрузка команды (графики через `components/pm/PmChart.vue` над chart.js). `PmLayout` отдаёт потомкам `provide('db', db)` — ССЫЛКУ, не строку: в потомках читать `db.value`; дети помечены `:key="db"` и пересоздаются при смене рабочей области. |
