...
| `WorkspaceNotFound.vue` | `/access-denied` (и `/workspace-not-found`) | Нейтральная страница «Рабочая область недоступна» для 403 и 404 DB_NOT_FOUND |
...
| `ObjectView.vue` | `/:db/tables/:typeId/:id` | Просмотр записи. `_value`/isPrimary-колонка читается из `obj.value.value` (не из requisites); ref-значения резолвятся через lookupOptions в формат `id:label` для CellRenderer. **Кружки зрителей записи** — см. ниже |
...
### ObjectView — кружки зрителей записи

Рядом с заголовком `<Тип> #<id>` показываются цветные кружки с первой буквой
имени тех, кто СЕЙЧАС открыл эту же запись; в подсказке — «<имя> смотрит эту
запись».

```js
const { viewers, join: joinPoint } = usePointPresence();
watch(objectId, (id) => { if (id) joinPoint('record', Number(id)); }, { immediate: true });
```

- Источник — WS-канал `point`, composable `usePointPresence` (`viewers` —
  `Map userId → { username, color }`). Себя в `viewers` нет.
- Цвет берётся из `getCursorColor(userId)` — те же цвета, что у курсоров
  соавторов в документах.
- Блок скрыт целиком при `viewers.size === 0`.
- Уход отправляется явным кадром `unsubscribe` из `leave()` composable'а
  (`onBeforeUnmount`); `WsClient.unsubscribe()` серверу не пишет.
- Экран — единственный потребитель канала `point`, и на это есть сторож
  `frontend/src/composables/__tests__/point-channel-has-client.test.js`: канал
  два месяца был мёртв именно потому, что клиента у него не было.
...
| `TgBotList.vue` | `/:db/telegram-bots` | List of workspace Telegram bots with CRUD |
| `TgBotEdit.vue` | `/:db/telegram-bots/:id` | Bot editor — config, commands, keywords, sync, status, test |
...
| `orgs/` | `OrgList`, `OrgEdit`, `OrgMembers`, `OrgSettings`, `OrgWorkspaces`, `OrgMyIssues`, `OrgPeople`, `OrgPortfolio` | Организации. `OrgEdit` — вкладки сводки PM по всем областям организации: портфель, мои задачи, команда, настройки, привязка областей. |
| `pm/` | `PmLayout`, `BoardView`, `ListView`, `BacklogView`, `SprintView`, `GanttView`, `CalendarView`, `MetricsView` | Задачи. Вкладки переключаются параметром `?tab=` (`board`, `list`, `backlog`, `sprint`, `gantt`, `calendar`, `milestones-roadmap` («Этапы»), `metrics`, `roadmap` («Дорожная карта»)). `MetricsView` — скорость, сгорание, накопительная диаграмма потока, загрузка команды (графики через `components/pm/PmChart.vue` над chart.js). `PmLayout` отдаёт потомкам `provide('db', db)` — ССЫЛКУ, не строку: в потомках читать `db.value`; дети помечены `:key="db"` и пересоздаются при смене рабочей области. `BoardView` — метки MultiSelect, отбор серверный (GET `/pm/board` принимает `labels`, TD-170); `IssueCard` — шеврон подзадач (`data-test="subtask-toggle"`, ленивая догрузка `?parentId=`, кэш до размонтирования). `ListView` — дерево: `topLevel` без детей, шеврон `data-test="row-toggle"`; при свёртке ключ УДАЛЯЕТСЯ из `expandedRows` (PrimeVue судит по наличию ключа, `false` строку не прячет — грабля-замер 01.09.2026). |
| `settings/` | `Profile`, `ChangePassword`, `Sessions`, `ApiKeys` | Настройки пользователя. `Profile` — аватар (загрузка до 2 МБ, сброс через `PATCH /iam/me` с `avatarUrl: null`), смена email через диалог (`currentPassword` + новый адрес, подтверждение ссылкой на почту), баннер ожидающей смены (`getPendingEmailChange`, адрес не раскрывается). `ChangePassword` — индикатор стойкости пароля, предупреждение об отзыве других сессий; текущая сессия сохраняется. `Sessions` — живые сессии (ip, user-agent, last used), метка «текущая», отзыв одной и «Завершить все другие», перечень ограничен 100 с припиской «ещё N» |
...
| `nightcall/` | `NightcallLayout`, `RunsPanel`, `RunDetail`, `ProposalsPanel`, `ProposalDetail`, `RequirementsPanel`, `RequirementEditor`, `IntentsPanel`, `SourcesPanel`, `CnmCanvas`, `ArtifactSpecEditor`, `ArtifactView`, `ArtifactNode`, `BaselinesPanel`, `FamiliesPanel`, `AuthorityPanel`, `OutboxPanel` | Nightcall — исполнение по спецификации (маршрут `/:db/nightcall`, модуль включается отдельно). Вкладки переключаются параметром `?tab=`: `runs`, `proposals`, `requirements`, `intents`, `sources`, `graph`, `specs`, `baselines`, `families`, `authority`, `outbox` (`runs` — умолчание, в адрес не пишется). |
