...
MCP-сервер превращает все 726 инструментов Integram в MCP-инструменты. Работает через stdio.
...
Пакет: [integram-mcp](https://www.npmjs.com/package/integram-mcp). Номер версии обязателен — плавающий `@latest` npx кэширует по строке запуска и может годами запускать старую сборку. О выходе новой версии сервер сообщает сам: строкой в `stderr` и припиской к ответу первого вызванного инструмента. Пакет выкладывается из CI по OIDC и несёт provenance-подпись — происхождение сборки проверяемо. Клон репозитория вместо пакета — `"command": "node", "args": ["/path/to/integram/mcp-server/index.js"]`.
...
| `search_tools("orgs")` | Организации: участники, приглашения, передача владения |
...
Агент понимает естественный язык, умеет создавать таблицы, заполнять данные, строить отчёты, запускать портал — всё через те же 726 инструментов.
...
| `GET /api/v2/openapi.json` | Полная OpenAPI-спецификация (тегов: 45, endpoints: 680) |
