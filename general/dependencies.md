...
| **PostgreSQL** | 17.x (pgdg) | Основная БД, EAV-хранилище (`_v2_objects`, `"db"."db"`) | Заменила MariaDB; ставится `scripts/setup-server.sh` из pgdg |
...
| `maxmind` | Чтение MMDB для геолокации IP в списке сессий (TD-183). База — DB-IP Lite Country, обновляется `backend/scripts/update-geoip-db.mjs`. IP geolocation by DB-IP.com (dbip-country-lite, CC BY 4.0) |
...
| `vue3-sfc-loader` | Runtime-загрузчик SFC для custom-code (кэш скомпилированных компонентов в localStorage, префикс `sfc:`) |
...
| `frappe-gantt` | Диаграммы Ганта (GanttView) |
