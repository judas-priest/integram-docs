...
| `corpus` | Сборка корпуса документации (npm run docs:corpus), заливка в `backend/.docs-corpus/` и загрузка в platform_docs на сервере; входит в `all` |
| `portal` | Build portal + sync .output + restart portal |
| `deps` | npm/bun install на сервере + restart all |
| `health` | PM2, расхождение прода с репозиторием, канал присутствия, сервисы, API health, диск, RAM, схема сателлитных таблиц (`check-side-tables-schema.mjs`, PM-225) и журнала автоматизаций (`check-automation-runs-schema.mjs`, PM-213) |
...
## Опциональные модули: внешние зависимости

Часть модулей opt-in (`settings.modules` воркспейса, по умолчанию выключены) и
требует на сервере своего окружения. Ничего из этого ставить не нужно, пока
модуль никому не включён.

### video-engine (обучающие видео)

Сценарий → запись экрана headless-браузером → TTS-озвучка → ffmpeg-монтаж → mp4.
Полное описание: `backend/docs/modules/video-engine.md`.

```bash
# браузер для Playwright (запись экрана идёт в headless chromium)
cd /opt/integram/backend && npx playwright install chromium

# TTS-движок (консольный скрипт edge-tts; неофициальный API Microsoft —
# при блокировке обновить пакет или переключить VIDEO_TTS_PROVIDER)
pipx install edge-tts        # либо: uvx edge-tts ... (запуск без установки)

# ДЕМОН НЕ ВИДИТ pipx-ПУТИ: у pm2/systemd свой урезанный PATH, без /root/.local/bin,
# и spawn edge-tts упадёт ENOENT. Симлинк в системный PATH решает раз и навсегда:
ln -sf /root/.local/bin/edge-tts /usr/local/bin/edge-tts
# (либо переменная VIDEO_TTS_BIN с полным путём — код читает её при каждом вызове)
```

Переменные (все с разумными дефолтами, полный перечень в доке модуля):
`VIDEO_BASE_URL` — база для navigate из сценария (дефолт `http://localhost:8081`),
`VIDEO_TTS_BIN` — путь к CLI, `VIDEO_RECORDER_CHROMIUM_ARGS` — доп. флаги chromium
(на машинах с GPU-крашками: `--disable-gpu --no-zygote`).

Включение в воркспейсе: настройки → модули → «video» (или `modules.video=true`
в settings). Артефакты джоб пишутся в `UPLOAD_DIR/<db>/video-engine/` и попадают
в бэкап воркспейса.
...
- **Миграции БД.** Обратных нет. Если правка меняла схему, откат кода оставит
  ...
  что записано после 3:00. Чек-лист восстановления — ниже, в разделе
...
`pg_restore` НЕ останавливается на недостающем расширении: он докладывает
«N ошибок проигнорировано» и молча пропускает TABLE DATA всех таблиц, чей тип
зависит от расширения (инцидент 29.08: кластер без `vector` — таблицы
создались, данные всех vector-таблиц потеряны, вскрылось через сутки).

Чек-лист (каждое восстановление, без исключений):

1. **Расширения — ДО `pg_restore`.** На приёмнике поставить всё, что есть у
   источника: `CREATE EXTENSION IF NOT EXISTS vector;` (и остальные из
   `SELECT extname FROM pg_extension` источника).
2. **После восстановления — сверка счётов:**

   ```bash
   node backend/scripts/verify-restore.mjs \
     --source postgres://.../источник \
     --restored postgres://.../приёмник \
     --schema <имя>
   ```

   `count(*)` по каждой таблице схемы + сверка расширений; любое расхождение —
   exit 1. На живом пишущем источнике добавлять `--tolerant` (источник ≥
   приёмник — предупреждение, а не отказ).
3. **Дамп с дефектной базы не считать копией** — он наследует потерю. Проверять
   каждое восстановление, а не дамп.

Скрипт не спасёт от «забыли запустить» — этот чек-лист второй слой.
