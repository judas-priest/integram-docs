...
> Аудит 06.09.2026: подтверждён и объяснён TD-139 — причина падения была DNS, не порты (см. аудит TD-139). Пункты 1–5 живы: круг по-прежнему не считает `_v2_grants` (grep по `scripts/template-roundtrip-check.mjs` — файл в корне репозитория — пуст).
...
> Аудит 06.09.2026: подтверждён по коду. `compareIssues` — `orgs/pm-order.js:37`; дублирование рангов не сведено.
...
## TD-189: Портальный маршрут удаления документа не существует — delta-документ с портала удалить нечем
