...
> Аудит 06.09.2026: подтверждён по коду (`findSecretsInText` требует присваивания `имя=значение`; `SECRET_KEY_RE` с `api[-_]?key` — `workspace-carry.js:207`). Заголовки с ключом по-прежнему срезаются целиком без плейсхолдера.
...
**Закрыт 19.09.2026:** ранги — `9d6de35f8` (`sqlPriorityCase()`); вторичные ключи (`updated_at DESC, number ASC`) — `issuesOrderBy()` в `pm-order.js`, литерал в `pm-aggregation.js` заменён; сторож «рукописный ORDER BY задач в orgs не остался» + golden-пин — `pm-issue-order.test.js`. Курсор (`cursorSql`) не переписывался — его защищает пин: правка порядка краснеет и заставит обновить курсор руками.
