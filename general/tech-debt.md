...
**Закрыт 21.09.2026.** Шаг 1 (автомаршрутизация) — 20.06.2026: meta-kb-agent в AGENT_EXAMPLES semantic-router.js. Шаг 2 (frontend) — коммит `7a116e7a4`: SelectButton-переключатель удалён из обоих компонентов (`AiPanel.vue` — вместе с мёртвым metakb-кодом: metaKbChat, welcome, дебаты, AgentSteps/MessageCards/CitationRenderer; `AiChat.vue` — только кнопка, режим жив через `?mode=metakb`: меню, DecisionDetail, deep links). Спеки-сторожа: `frontend/src/components/__tests__/AiPanel.spec.js`, `frontend/src/views/ai/__tests__/AiChat.spec.js` (краснота доказана до правки).
...
~~**portal**~~ — **ЗАКРЫТО 20.09.2026 (TD-012b)**. Staff-мутации заказов закрыты 10.09.2026 (`add_portal_order_item`, `collect_portal_order_items`, `merge_portal_orders`). Клиентские мутации закрыты шестью admin-tools: `add_portal_cart_item`, `update_portal_cart_item`, `remove_portal_cart_item`, `clear_portal_cart` (HIGH, HITL), `create_portal_ticket`, `create_portal_order` (HIGH, HITL + idempotencyKey); создание тикета вынесено из инлайна маршрута в `portal/service.js createTicket` — маршрут и tool зовут одну реализацию. By-design НЕ покрыто (приватные/безличные/инфраструктурные): `PUT /api/profile`, `PUT/DELETE /api/account/llm-key`, `POST /api/cart/merge` (миграция анонимной корзины при логине), `POST /api/orders/guest`, `POST /api/analytics/event` (fire-and-forget), share/unshare бесед, `invoke-many`, `agent/resume`. В клиентский каталог чата не публикуются — видимость только рабочее место админа/MCP.

> Аудит 06.09.2026: частично устарел — «мутаций заказов нет» неверно с 10.09.2026 (см. выше). swarm-memory и by-design-модули — без изменений.
...
#### ~~5. SQL интерполяция вместо ? плейсхолдеров~~ ЗАКРЫТ (де-факто)

**Верифицировано:** TRUE. Template literal interpolation в objects/service.js (aggregate/pivot/grouped SQL).

Все интерполируемые значения защищены: `colId` — через parseInt (integer), `fn` — через whitelist `ALLOWED_FUNCS`, `safeDb` — через `eavTable()` sanitizer.

> ЗАКРЫТ 19.09.2026 (де-факто, план `docs/superpowers/td-plans/TD-018.md` п.5): значения давно параметризованы через `?`; интерполяция осталась только для НЕпараметризуемых идентификаторов (`${safeDb}`, whitelist-функции, `col_${colId}_${fn}` из parseInt+whitelist). Править нечего; вынесение разбора `colId:FUNC` в общий хелпер — только при ближайшем рефакторинге aggregate/pivot.

#### ~~6. Dashboard/frontend — silent catches на загрузках данных~~ ЗАКРЫТ 19.09.2026

**Частично исправлено.** DashboardBuilder.vue data-load catches теперь имеют `console.error`.

> ЗАКРЫТ 19.09.2026 (план `docs/superpowers/td-plans/TD-018.md` п.6): все 6 мест получили `console.error` — DashboardBuilder.vue (`toggleReportColumnHidden`), AutomationEdit.vue (:196 type-fields, :434 batch-poll), GraphExplorer.vue (:297 health, kagStats `.catch`, kagAnomalies). Уточнение: утверждение плана про «неоткат оптимистичного update» устарело — `col.hidden = hidden` ставится ПОСЛЕ await (не оптимистично), отката не требуется. Grep-проверка: в трёх файлах не осталось `catch {}`/`.catch(() => …)` без `console.error`/toast.

### P3 — Низкий риск

#### ~~7. req.body деструктуризация без || {} (~23 места)~~ ЗАКРЫТ 19.09.2026 (с остатком — pass-through)
...
> ЗАКРЫТ 19.09.2026 (план `docs/superpowers/td-plans/TD-018.md` п.7). Все прямые обращения `req.body.prop` защищены: meta-kb/router.js — деструктуризация `|| {}` (:91) + `req.body?.label` / `req.body?.entityIds`; ai/router.js — dlp-PATCH (`req.body?.[k]`, `rule_type`, `severity`) и `pageContext` ×2; portal/router.js — `objectId`, `qty`, `convId`; files/router.js — `/mkdir` переведён на `req.validated.body` (канон п.4), upload — `req.body?.subdir/objectId`; swarm-memory/router.js — `req.body?.tag`. Red-first: `meta-kb/__tests__/router-body-guard.test.js` — PATCH /iterations/:id без Content-Type даёт 400 «status is required» (до правки — 500, краснота доказана). Остаток — pass-through `req.body` целиком как аргумент службы (portal :198 telegram-webhook, :3488/:3516/:3648/:3682, meta-kb :130/:189, ai :592): внешний вызов с JSON content-type, риск низкий — осознанно не тронуто.
...
> Переформулировано 19.09.2026 (план `docs/superpowers/td-plans/TD-018.md` п.8): перечисленные выше 6 мест ЗАКРЫТЫ давно (одиночный unwrap с комментарием-контрактом). Пачка 19.09: 7 мест с 3+ fallback переведены на `res.data ?? []` со сверкой контракта бэка — useSavedViews.js:18, useResolvedCellValue.js:42 (+ спек-контракт), ObjectList.vue:2399/:2427, ObjectEdit.vue:118, AddTabDialog.vue:61 (documents `{items,total}`), IssueDetail.vue:95 (schema envelope). useAIChat.js `res.data.data` — НЕ двойной unwrap (прямой axios-вызов, конверт `{ok,data}` бэка `/ai/conversations`) — оставлено. Остаток: ~два десятка двухсторонних `res.data || res` (79 по замеру 13.09) и несколько 3+-цепочек вне перечня плана (usePortalEditor.js:488, useSchemaOptions.js:54, DashboardBuilder.vue:201, DashboardList.vue:23, TgBotEdit.vue:108, IntegramTableEmbed.vue:25, RecordPicker.vue:20 и др.) — чинить при касании файлов со сверкой контракта маршрута.
...
**~~ORDER BY без числового CAST~~** — ЗАКРЫТО (2026-09-21, план `docs/superpowers/td-plans/TD-023.md`): в `report-engine.js` заведён `numericOrderExpr` — тот же regex-ограждённый CASE (нечисловое → NULL → NULLS LAST), что у агрегатов движка, для NUMBER/SIGNED/CURRENCY/PERCENT/DURATION/RATING/AUTO_NUMBER в неисполнимых-агрегатах отчётах; применяется после `dateOrderExpr` в обоих путях сортировки. В агрегатных отчётах сортировка остаётся текстовой (граница псевдонима). Тест: `backend/src/api/v2/utils/__tests__/report-engine-orderby-cast.test.js` (форма SQL) + `report-engine-orderby-cast-pg.test.js` (исполнение: 7, 9, 10, «8a» в конец).

**~~mainCol исключена из GROUP BY~~** — ЗАКРЫТО (2026-09-21, план `docs/superpowers/td-plans/TD-023.md`): видимая mainCol добавляет `a.val` в GROUP BY (агрегированная mainCol — по-прежнему нет); наследие `407691329` (ветки ref-колонок в GROUP BY) не тронуто — сторож `report-engine-orderby-cast-pg.test.js`, агрегатный отчёт с ref+SUM.
...
3. ~~**POST-роуты отвечают 200 вместо 201**~~ — ЗАКРЫТО (2026-09-17): 8 создающих POST (`requirements`, `versions`, `edges`, `intents`, `sources`, `proposals`, `artifact-specs`, `decisions`) теперь `res.status(201)`; 4 действия (`outbox/drain`, `proposals/:id/decide`, `resolve`, `run`) остались 200. Заслон — `__tests__/router-status.test.js` (12 тестов, 8×201 + 4×200).
- Файл: `backend/src/api/v2/modules/nightcall/router.js`
- Приоритет: низкий (все пункты закрыты)

> Аудит 06.09.2026: пункт п.3 стал ХУЖЕ описанного — модуль теперь НЕСОГЛАСОВАН: POST-роуты уже отвечают 201 (router.js:154, 205, 221, 237 и — пропущенный в аудите — 370, `POST /runs/:id/view`; всего ПЯТЬ), остальные 13 — `res.json` (200). «Менять только вместе» уже нарушено наполовину.
...
## ~~Nightcall proposals — терминальный apply_failed~~ — ЗАКРЫТО (2026-09-19) — план docs/superpowers/td-plans/TD-nc-apply.md
...
- Приоритет: низкий
- ЗАКРЫТО: весь `decideProposal` собран в один `withTransaction` (claim → решение → apply → финальный UPDATE → recordEvent); при сбое ROLLBACK возвращает proposal в исходный статус, ретрай разрешён. `apply_failed` остаётся в `PROPOSAL_STATUSES` ради легаси-строк — новые не возникают. Застрявшие легаси-строки — разовая ручная процедура из раздела «Риски» плана.
...
## ~~TD: upsertConfig затирает custom_domain при апдейте без него~~ — ЗАКРЫТО 17.09.2026 — план docs/superpowers/td-plans/TD-upsert.md
...
> ЗАКРЫТО 17.09.2026 — план docs/superpowers/td-plans/TD-upsert.md. Контракт RFC 7386 (undefined — колонку не трогать, null или '' — очистить, строка — как есть) через COALESCE + domainArg в обеих ветках upsertConfig; сторож — `upsert-config-guards.test.js`, describe «custom_domain не стирается умолчанием».
...
> Исполнение 19.09.2026 (план `docs/superpowers/td-plans/TD-027.md`, ветка worktree agent-a8d79748, не влито). Идея 1 (параллельный кросс-дебат) была исполнена ещё `68036226b` от 2026-08-06 — в тот же день, что заведён TD. Исполнены шаги A-C: (A) `mk_start_debate` возвращает короткий результат `{consensus, debateId, stats}` — полный протокол в `_v2_mk_debates`, доступен через `mk_get_debate(debateId)`; (B) `MAX_DEBATE_ROUNDS` 5→3; (C) параметр тула `fast` — заседание без кросс-фазы. Тайминги «17 минут» — из записи живого заседания, НЕ воспроизведение; профиль изменился после параллелизации. Отдельный множитель вне этой работы: loop mode (`runDebateLoop`, до 3 полных прогонов дебата) — по умолчанию выключен. Ручной wall-clock-замер до/после на dev-стенде не проводился.
...
> Аудит 06.09.2026 (живой скан прод по команде из записи): у `uav/1.git` дерево `/tmp/uav-edit` зарегистрировано со статусом **prunable** (каталог в /tmp вычищен); в `uav_pre_restore_20260901/1.git` — второе, тоже prunable. Незакоммиченных правок на диске больше нет; осталось `worktree prune` в обоих.
...
> **Исполнено 21.09.2026 (план `docs/superpowers/td-plans/TD-030.md`, на ветке — НЕ выложено).** Все восемь учтённых мест закрыты: пропагация статуса (`handleOpU`/`handleOpScx`) сведена в `propagateStatusToLinked` и читает `orderStatusReqId`, `propagateStatuses` (новый ключ), `terminalStatuses` (новый ключ, заменяет зашитую константу 398/399) из конфига СДЭК-коннектора; превью/подтверждение батч-отгрузки (`showCdekBatchPreview`/`handleOpCbc`) читают `ordersTypeId`, `orderStatusReqId`, `collectedStatusId` (новый ключ), `recipientNameReqId`, `weightReqId`, `recipientPhoneReqId` — фолбэк `|| 423` снят; `renderScreen`-`stockLookup` стал объектом с номерами (`productTypeId`…`invDateReqId`), старый boolean отвечает warn; места 1/2 (`toggleItemCollected`/`collectAllItems`/`checkAllCollected`/вставка позиции в `addOrderItem`) читают шесть номеров из раздела `staff` портального конфига, раздела нет — `AppError NOT_CONFIGURED`. Краснота доказана red-first (`telegram-callback-config.test.js`: 14 красных assert'ов на неисправленном коде), characterization на сеяном fund-demo-конфиге зелёный. **Перед выкладкой сеять в fund-demo:** в конфиг коннектора `collectedStatusId: 394`, `propagateStatuses: [395..399]`, `terminalStatuses: [398, 399]` (PATCH полным конфигом); раздел `staff` в портальный конфиг (POST /api/config полным конфигом); `stockLookup`-объекты в конфиги экранов автоматизаций (PATCH полным actions_cfg). **Остаток того же класса (в восьмёрку не входил, план TD-030):** `handleOpDi` (габариты 346-349 — ключи в конфиге есть, не читаются), `handleOpLom` (фолбэки 340/378/'399'), гостевой заказ `handleOpNos` (311/338/423/339/373/374/378, поиск «Новый» по имени в t=302), поиск заказов (ФИО 338, телефон 423), агрегированная доска сборки (`toggleReqId 363`), кнопки экранов (`checkReqId || 378`, ×10), `createShipment(..., 311)` в связанном цикле отгрузки (`telegram-callback.js` ~:2112), остатки `staff-orders.js` (328/381/361/362/374/337-350/375-376/391-399, `linkedOrderReqId 3949070` в `findLinkedOrders` — расходится с `linkedOrderReqId` конфига), `portal/router.js` product-variants (131642/131644/131645).
...
> Аудит 06.09.2026: состояние «закрыт частично» подтверждено по описи в самой записи. Открытый остаток (`.btn-primary`/`.btn-secondary` глобально, 29 неограниченных правил пользовательского кода) — решение владельца, кодовых изменений с 07.08 не видно.
...
> Аудит 06.09.2026: закрытие подтверждено. `.claude/rules/frontend.md`, gotcha `[primevue/import]` — правило переписано: резолв автоматический, явный импорт — стилистический выбор.
...
> Аудит 06.09.2026: подтверждён. `portal.css:227` (`.btn-primary:disabled { opacity: .6 }`), `:315` (`.btn-telegram`), `:915` (`.chat-send:disabled { opacity: .5 }`) — приглушение прозрачностью в оболочке на месте.
...
> Аудит 06.09.2026: закрытие подтверждено. `buildListOrder` (`ai/agent/tools/objects.js:504`) и `buildListWhere` (`:441`) стоят в самом запросе до LIMIT (вызовы `:593,612`; комментарий «сняты… до LIMIT» `:642–643`).
...
> Аудит 06.09.2026: закрытие подтверждено (один `buildListWhere` кормит выборку/счётчик/агрегаты). Хвост про summary (только перевёрнутый способ хранения ссылок) жив и зафиксирован платформенным готчей `[objects/summary-одна-форма]`.
...
> Аудит 06.09.2026: подтверждён на кодовой части. Голые `CAST(val AS DECIMAL(20,6))` без регексп-гварда — `utils/computed-reqs.js:438,445,546` (падение на живой колонке 381 в usadba_3 — прод-данные, в этом проходе не воспроизводилось).
...
> Аудит 06.09.2026: закрытие подтверждено. `assertMoveSafe` + `MOVE_WOULD_STRAND_DATA` — `workspaces/workspace-move.js:23,64`.
...
> Аудит 06.09.2026: закрытие подтверждено. `utils/db-scoped-cache.js` существует, `poolScopedMemo` импортируется (`lazy-init.js:39`), `invalidateDb(data.db)` зовётся на сносе (`v2/index.js:333,346`).
...
> Аудит 06.09.2026: закрытие подтверждено. `active = true` в деструктуризации, колонка в INSERT, комментарий «active едет в событии» — `automations/service.js:269–287`.
...
> Аудит 06.09.2026: закрытие подтверждено. `dispatchPmTrigger` делегирует `dispatchEvent` (`pm-triggers.js:37`), четыре pm-типа в whitelist (`automations/router.js:35`).
...
> Аудит 06.09.2026: подтверждён. `PROTECTED_SUFFIXES` — ровно четыре пути (`portal-auth.ts:11`); `requireAuth` в portal/router.js — 0 вхождений, ключ конфига по-прежнему мёртв. Анонимный 200 на `/portal/api/tables/...` не воспроизводился в этом проходе (замер 13.08 в записи).
...
> Аудит 06.09.2026: подтверждён по коду. 403-перехватчик (`services/api.js:51+`) и `onWorkspaceAccessDenied` → редирект (`main.js:82`) на месте; пометки причины, по которой компонент молчал бы, нет — тосты дублируют страницу отказа.

---

## TD-046: агрегаты вычисляемых реквизитов складываются в JS, а не в базе

**Исполнено 21.09.2026 (worktree, план `docs/superpowers/td-plans/TD-046.md`):** SUM/AVG/MIN/MAX переведены в SQL-агрегаты (`GROUP BY` по родителю; в useUp-ветке — JOIN со строкой-объектом ребёнка, в link-ветке — с UNION обоих ref-паттернов), `NULLIF(val,'')` перед кастами закрывает и точки TD-039 №2–3; в ответе — число (один `Number()` на границе, предупреждение за 2^53); `listComputedReqs` отдаёт `outputType` (COUNT→13, CONCAT→3, FORMULA→15, SUM/AVG/MIN/MAX — тип целевой колонки из {13,14,1002,1003}, иначе 13), клиент — `def.outputType ?? 15`. Живой вызов usadba_3 (шаг 6) и выкладка — не сделаны (работа в worktree); триггерная проверка прод-данных не прогонялась — исполнение по явной команде.
...
> Аудит 06.09.2026: подтверждён. JS-агрегация на месте — `vals.reduce` в двух ветках (`utils/computed-reqs.js:458–459` и `:559–560`), построчный SELECT с CAST (`:438,445,546` — заодно мины TD-039).
...
> Аудит 06.09.2026: закрытие подтверждено. Сторож `tool-tiers-coverage.test.js` существует; `updateTool` пересчитывает тир (`workspace-tools/service.js`, `risk_tier = ?` в UPDATE :424); `toolRiskTier` — `risk-tiers.js:692`.
...
> Аудит 06.09.2026: закрытие подтверждено. `pg_advisory_xact_lock(hashtext('pm_reparent:<db>'))` — `pm/service.js:525–526`; тест `pm/__tests__/reparent-lock.test.js` на месте.
...
> Аудит 06.09.2026: закрытие подтверждено. Адресация по `->>'id'` в `toggleChecklistItem` (`pm/service.js:869,881,887`); тест `checklist-item-id.test.js` на месте.
...
> Аудит 06.09.2026: закрытие подтверждено. Разовая миграция `pm_fix_orphans` — `pm/schema.js:166` (идемпотентная, в ensureTables); тест `orphan-fix.test.js` на месте.
...
> Аудит 06.09.2026: закрытие подтверждено. `hasVelocity`/`hasCfd` по наличию наборов (`MetricsView.vue:100–101`), сообщение «Нет завершённых спринтов…» (`:139–140`); спек `MetricsView.spec.js` на месте.
...
> Аудит 06.09.2026: подтверждён косвенно, без пересчёта 105 воркспейсов. Живой пример из этой же сессии: `fund-app/app.vue` ввозит `@kit` ради единственного `AiPanel` (динамический импорт `:362`), остальная геометрия/парсинг — свои. Против записи ALSO: план сведе́ния смыслов (п.1) с 05.09 не начат.
...
> Аудит 06.09.2026: подтверждён частично. `DataTable.vue` — `formatOf` с именованием ошибки присутствует (:121–151); правок по словарям узлов и разовому сторожу `ui` с 14.08 не видно. Приоритет «кроме словаря имён узлов» в силе.

---

## ~~TD-054: `page-layout.vue` фонда не показывает иконку раздела~~ ЗАКРЫТ
...
> Аудит 06.09.2026: не перепроверялся — файл в codespace-репозитории фонда; чтение через API в этом проходе содержимого не отдало (перепроверить в выкладочное окно).

> **ЗАКРЫТ 22.09.2026:** дефект подтверждён чтением файла (baseCommit 6d42a7ff), исправлен коммитами в codespace `fund-app` (main): `5955c434` — слот `#icon` рисуется независимо от `live` (точка live сохранена); `1b3736b777` — `live-hint` передан в `capital-calls`/`gr`/`waterfall`. Инцидент записи зафиксирован и отработан: первый коммит (`031f83ff`) ушёл в base64-литерале (в commit_file нет поля encoding) — восстановлен побайтно из сохранённого слепка (`d58b49b2`), потом наложен фикс. Живая проверка на портале `#/gr`: иконка 18px видна, точка live на месте.
...
> Аудит 06.09.2026: подтверждён по снимку `app.vue` от 05.09 — `toFixed` ×5, `pct(` ×5; полный пересчёт «20 мест» не делался.
...
> Аудит 06.09.2026: закрытие подтверждено по коду: указатель в `MEMBERSHIP_UNIQUE` (`registry/tables.js:301,342`), `clearUserRoleCache` при передаче владения (`orgs/service.js:188–189`), `viaWorkspaceOnly` (`:125,371`).
...
> Аудит 06.09.2026: закрытие подтверждено. Сторож — `swarm-memory/kag.js:221`, вызовы `:254,313`.
...
> Аудит 06.09.2026: закрытие подтверждено в master. Прокси-роутер смонтирован (`portal/router.js:3997`), `POST /` с rate-limit и requirePortalJwt (`decisions-proxy.js:7–32`), сторож `decisions-proxy-post.test.js` упомянут в самом файле. Состояние стенда nti-new не перепроверялось (другой сервер).
...
> Аудит 06.09.2026: подтверждена половина — `by-type` в master НЕТ (grep пуст, как и утверждает запись); жив ли маршрут на стенде nti-new сейчас — не проверялось (другой сервер, требует SSH-прохода).
...
> Аудит 06.09.2026: закрытие подтверждено. `unwrapEnvelope` — `usePortalDataLayer.js:87`, применён в `ensureUser` (`:102`); сторож `portal/test/data-layer-user.test.js` на месте. Смежный хвост (сырой конверт в useCollection и др.) не трогался.
...
> Аудит 06.09.2026: подтверждён. `getUser() { return user; }` без запуска чтения — `useCustomCodeApi.js:438`, не изменился после фикса TD-060.
...
> Аудит 06.09.2026: подтверждён ЖИВОЙ страницей. `https://ai2o.online/promprib/portal/auth` — SSR-HTML 5,7 КБ, вхождений «Вход для сотрудников» — 0; в мастере кнопка на месте (`portal/pages/[db]/portal/auth.vue:208`). Выкладка портала по-прежнему старше мастера.
...
> Аудит 06.09.2026: подтверждён. `case "editor_insert_heading"` по-прежнему `return JSON.stringify({ editorOps: … })` без серверной ветки (`tool-executor.js:898+`).
...
> Аудит 06.09.2026: подтверждён. Все четыре точки живы — `ai/router.js:336,367,1274,1382` возвращают `ok: true` с `data.error` внутри.
...
> Аудит 06.09.2026: подтверждён. Проглатывание и `ctx._scriptErrors` на месте (`execution.js:1219`, журнал `:154–155`); `_script_result` в окружение при сбое по-прежнему не кладётся — цепочка доигрывает на пустоте.
...
> Аудит 06.09.2026: закрытие подтверждено. `objectWhere` в `sql-guards.js` несёт условие `(up = t OR up NOT IN (…up=0 AND id<>t))` — определения под записью-типом отсечены.
...
> Аудит 06.09.2026: закрытие подтверждено. `writeMultiRefRequisite` — `eav-ref.js:150`, зовётся из общего пути (`:230`); тест `multi-ref-requisites.test.js` рядом с objects.
...
> Аудит 06.09.2026: подтверждён. `bus.*` в `files/` и `normalizer/` — по-прежнему пусто; `broadcastUpdate` в `doc-processor.js` — 30 вхождений.

---

## ~~TD-070: из песочницы автоматизаций роль всегда `admin`~~ ЗАКРЫТ

> Исполнено 20.09.2026 (план `docs/superpowers/td-plans/TD-070.md`): повышение объявлено явно —
> `elevated: true, reason: 'automation', automationId` в `execution.js` (run_script, актуальная
> строка была :1309), `run-server-function.js:150` (с сохранением roleId из PM-125, пометка
> `source` снята) и `batch.js` (run-batch, устранено расхождение с единичным прогоном); заслон
> `requireAdminForKagWrite` (`isolated-runner.js`) принимает метку и пишет
> `[sandbox] kag_write under automation elevation` в журнал. Замечание про editor-автора устарело:
> автоматизацию кладёт только admin (router.js:191, TOOL_MIN_ROLE).
...
> Аудит 06.09.2026: подтверждён, строка уехала: `scriptUser = { ...ctx.user, role: 'admin' }` — `automations/execution.js:1185` (в записи :1142). Безусловное повышение на месте.
...
> Аудит 06.09.2026: закрытие подтверждено. Маршрут портала зовёт общую `filesService.queueFileProcessing` (`portal/router.js:3124`).
...
> Аудит 06.09.2026: подтверждён. Читатели `:ALIAS=` на месте (schema/service.js, objects/service.js ×6, ai/agent/tools/objects.js ×7 — счёт grep'ом); починки с 18.08 не видно.
...
> Аудит 06.09.2026: подтверждён. `processed = expr.replace(/\[([^\]]+)\]/g, …)` — `formula-engine.js:413`, дословно из записи.
...
> Аудит 06.09.2026: подтверждён на половине «автоматизации». Полный список зовущих `evalComputedReqsBatch`: portal, objects, ai-tools, report-engine (+ сам computed-reqs) — в automations его нет, как утверждает запись. Половина про отчёты не перепроверялась.
...
> Аудит 06.09.2026: ЧАСТИЧНО закрыто без отметки. `appendBlock` (`ai/agent/tools/documents.js:170–185`) теперь разобраёт markdown у входа (`looksLikeMarkdown` → `markdownToDelta`) и умеет `type: 'heading'` — заголовки, списки и таблицы через строку проходя́т. Непокрытым остался ввод готовой дельты (`format: 'delta'`) — тихая подмена дельты в абзац по-прежнему возможна.
...
> Аудит 06.09.2026: подтверждён. Рекурсивный `deepMerge` в этом файле относится к `set_portal_config(merge:true)`; сам `update_portal_module` мержит spread-ом на один уровень (`{ ...modules[mi].config, ...moduleConfig }`) — вложенный `bindings` по-прежнему заменяется целиком, молча.
...
> Аудит 06.09.2026: подтверждён по коду. `clientObjectId` читается из токена (`portal/router.js:1137` и др.) без перепроверки живости записи; решение с признаком `clientUnlinked` не реализовано.
...
> Аудит 06.09.2026: подтверждён повторным замером тем же способом (сверка `def.description` регекспом по TOOL_DEFS × каталоги 24 агентов): пар 276 (было 430), разошедшихся 160 (было 299). Расхождение сократилось (~40 %), но живо: правки описаний продолжают идти в одну из двух копий (пример — cd8e8d660, менял только TOOL_DEFS). Метод замена грубее исходного (многострочные описания недосчитываются), абсолютные числа — оценка, вывод устойчив. Описание — единственное, из чего модель узнаёт состав
...
> Аудит 06.09.2026: подтверждён. Молчащие `.catch` на месте: посевные `demo_seed_multi_base:4291`, `demo_seed_multi:4303`, `demo_seed_ref:4350`, `demo_seed_field:4361`, документные `tpl_apply_doc:3486` (`.catch(() => ({}))`), `ws_tpl_insert_doc:4794` (строки уехали с записанных).
...
> Аудит 06.09.2026: подтверждён. Запрос «хоть один бот» жив — `workspace-templates.js:2451` (строка уехала с :963). Смягчённый текст причины не перепроверялся.
...
> Аудит 06.09.2026: подтверждён. Граница `\d{2,7}` на месте — `workspace-templates.js:1052` (в записи :236, файл вырос). Замер MAX(id) по областям не делался.
...
> Аудит 06.09.2026: подтверждён. `includeContent && includeData` гейтит карту номеров (`workspace-templates.js:1965`), `portable: scriptIds.size === 0` (`:2286`) — оба кодовых якоря на месте (строки уехали с :641/:890).
...
> Аудит 06.09.2026: закрытие подтверждено с поправкой пути: скрипт лежит в КОРНЕ репозитория — `scripts/template-roundtrip-check.mjs`, а не `backend/scripts/` (замер `find`). Повторный излом в этом проходе не ставился.
...
> Аудит 06.09.2026: подтверждён по цепочке. `_v2_dlp_rules` в реестре с числовой принадлежностью `workspace_id` (`workspace-carry.js:1917` — потому общий переносчик и не может везти её вслепую), `llm-router.js:485` — включение по `dlp?.enabled`, `contour-guard.js:200` — пустой перечень → `blocked: false`. Замер «в базе 0 правил» не повторялся.
...
> Аудит 06.09.2026: подтверждён. Замер на текущем master: `template.mode: manifest` — ровно 10 записей, как заявлено сведением 21.08.
...
> Аудит 06.09.2026: подтверждён повторным замером. Перечни по-прежнему совпадают (120 против 120, разность множеств пуста), сторожа нет — grep по `reservedSlugs|reserved-slugs` находит только сами файлы и читателей плюс `apm-diag.js` (третье чтение, в записи не названо).
...
> Аудит 06.09.2026: подтверждён. `migrate-legacy.js:39` — `mysql2/promise` и `INSERT` без `slugToDbName` (`:519`); `import-legacy-workspace.js` имени через сторож не получает. Скрипты в этом проходе не исполнялись — суждение по коду.
...
> Аудит 06.09.2026: подтверждён. Две реализации на месте: `workspaces/service.js:135` и `middleware/apm-diag.js:56`; сведение не состоялось.
...
> Аудит 06.09.2026: подтверждён. Литерал `'integram-sk'` в выдаче (`workspace-bots.js:14`), отказ без литерала в проверке (`service-key-auth.js:20`) — оба конца без изменений.
...
> Аудит 06.09.2026: подтверждён по коду; живой прогон `import('mariadb')` в этом проходе не повторялся (замер в записи — `ERR_MODULE_NOT_FOUND`). Соседний `migrate-users-to-v2.js` не проверялся.
...
> Аудит 06.09.2026 (живой замер на проде): сирот в `_v2_ai_audit_log` стало 10 (было 2660) — основную массу прибрало; класс жив, приборка отдельным скриптом с исключениями по-прежнему не сделана. `__platform_docs__`-оговорка остаётся существенной.
...
> Аудит 06.09.2026: подтверждён. «Честность» на месте: `manifest.secrets.uncut` (`apm-diag.js:193,498`) и сторож `middleware/__tests__/apm-diag.test.js` существуют; само вырезание тайного из EAV не сделано, как и записано. Живой замер «1 область с токен-указателем» не повторялся.
...
> Аудит 06.09.2026: подтверждён. Образец `SECRET_KEY_RE` без изменений (`workspace-carry.js`); живой замер на проде вместо дев-базы: `config::text LIKE '%credentials%' OR '%apikey%'` по `_v2_portal_config` — 0, форма auth коннектора в вырезаемых конфигах по-прежнему не встречается, дорога закрыта только тем, что EAV не вырезается (TD-104).
>
> **Закрыто 20.09.2026 (план TD-106).** `CONNECTOR_AUTH_TYPES` (`bearer, basic, apikey, google_service_account, header`) объявлен в `registry/workspace-carry.js`; `stripSecretValues` режет по КОНТЕКСТУ: узел с `type`/`authType` из перечня — целиком, плоская форма теряет `authValue`. Перечень выведен из читателей ОБОИХ модулей, сторож полноты — разбор их исходников в `carry-invariants.test.js`; «сторона 3» и R28-тест клона переписаны на «ветка вырезана целиком». 7 строк UDS больше не проносят `credentials`. Связь с TD-104: после его исполнения R28-проход клона станет перекрыт сверху — не удалять без замера.
...
> Аудит 06.09.2026: закрытие подтверждено. `rolesCreated`/`rolesForeign` в коде apply — `workspace-templates.js:3699,3704,3752`.
...
> Аудит 06.09.2026: замер исполнен — `Object.keys(TOOL_MIN_ROLE).length` → 49, совпадает с ADR-027. Пункт 1 (две реализации) и п.2 (корзина, backlinks, реакции, агрегаты) живы — правок с 21.08 не видно.
...
> Аудит 06.09.2026 (живой замер на проде): `config::text LIKE '%telegramBotToken%'` по `_v2_portal_config` — 0; перечень пуст, ротировать нечего. Скрипт `portal-secret-audit.mjs` на месте. Долг открыт по тем же двум причинам (засвет навсегда + будущее редактирование).
...
> Аудит 06.09.2026: подтверждён. Остаток жив: `webhook_secret` в ответе инструмента (`ai/agent/tools/portal.js:679`); `PUBLIC_ROW_COLUMNS` в `upsertConfig` на месте (`portal/service.js:136`) — вторая дверь держит закрытой.
...
> Аудит 06.09.2026: подтверждён. Сторожа живы — `blob-route-access.test.js`, `code-studio-access.test.js`; в коде оба снятых места помечены «измерено, а не забыто» (`portal/router.js:3161`, `:4700`). Живой замер «7 из 17 разделов при codespace:false» не повторялся.
...
> Аудит 06.09.2026: подтверждён. Оба якоря на месте: `isBlobFileAllowed` (`code-guard.js:123`), `validateFilePath` (`codespace/service.js:121`); правки заслона с 22.08 не видно.
...
> Аудит 06.09.2026: подтверждён по коду без живого репо-замера: ветка браузера `module.config.ref || 'main'` и заслонная `|| repo.default_branch || 'main'` не сходились и в 22.08-замере; сторож `codespace-blob-exposure.mjs` на месте. Спит, пока все боевые дефолт-ветки зовутся main.

> **Закрыт 20.09.2026 (обе ветки, остаток выделен в TD-113a).** Ветка 1: конфиг-ответ `GET /:db/portal/api/config` несёт `data.defaultBranches` (`router.js`, метка `portal_config_default_branches`), экран считает ref одним местом `resolveModuleRef` (`portal/components/modules/module-ref.js`; `ModuleCustomCode.vue` и `staff.vue` больше не шлют жёсткий `'main'`); заслон не тронут. Ветка 2: блоб-роут разводит по `user_type` — сотруднику роль по членству (`resolveStaffWorkspaceRole`), клиенту по-прежнему `loadClientRole`; выбывшему из воркспейса пол `'viewer'` отказывает. Тесты — `blob-route-access.test.js` (36 зелёных) и `portal/test/module-ref.test.js`; краснота обоих красных кейсов доказана прогоном до правки. **Остаток TD-113a:** экранная половина роли — `ensurePageAccess` (`router.js`, middleware перед страницами, `loadClientRole(pc.clientObjectId)` на `page.roles`) — сотруднику без клиентской записи по-прежнему отказывает; тот же класс, но отдельное решение про видимость страниц.
...
> Аудит 06.09.2026: подтверждён. `pageRoles.filter` в заслоне — `code-guard.js:87`, роли читаются как массив без приведения; путь наложения шаблона (прямой INSERT) по-прежнему не обвешан `validatePortalConfig`.
...
**РЕШЕНО 20.09.2026:** нормализация на чтении (`toRoleArray` в
`config-utils.js`; заслон `code-guard.js`, `filterByRole` в `roles.js`,
`getVisibleModules`) плюс валидация `validatePortalConfig` на обоих шаблонных
INSERT (`workspace-templates.js`, наложение и заведение области — дефект
называется через `portalFailed`). Сторожа: `blob-exposure.test.js`
(строка не роняет и не вырождается в буквы), `roles-normalize.test.js`,
`workspace-templates-portability.test.js` (роль строкой не проходит шаблонный
путь молча).
...
> Аудит 06.09.2026: подтверждён по коду. `GET /admin/export-all` — `admin/router.js:178`, обычный `ok(res, …)`; живой вызов клиентом с blob не повторялся.
...
> Аудит 06.09.2026: подтверждён. `String(row.val || '')` в коде — `backup-carry.js:392`; различение NULL/'' теряется. Живой замер «10 строк в 7 областях» не повторялся.
...
> Аудит 06.09.2026: подтверждён. Фиксированные 48 символов — `backup-carry.js:1155`; `character_maximum_length` в файле не спрашивается (grep пуст). Живого случая узкой колонки нет.
...
> Аудит 06.09.2026: подтверждён по коду. `writeUInt16LE(entries.length, …)` в хвосте оглавления — `admin/zip.js`; заслона на число входов нет. Живой случай по-прежнему недостижим.

---

## ~~TD-119: «прежние ссылки мертвы» недоговаривает про бота — приём входящих гаснет молча~~ ЗАКРЫТ
...
> Аудит 06.09.2026: подтверждён. Текст сообщения — `admin/service.js:329`; ранний `res.sendStatus(200)` на приёме вебхука — `portal/router.js:185` (неизвестный секрет гаснет в журнале, Telegram повторов не шлёт).
...
> Исполнено 18.09.2026 (план `docs/superpowers/td-plans/TD-119.md`). Случайная
> выдача для `restore.external` реестра (`_v2_tg_bots`: `token`,
> `webhook_secret`) заменена меткой утраты
> `внешняя-тайна-утрачена:_v2_tg_bots.token#<id>`, из которой секрет
> досчитывается `sha256(метка).slice(0,32)` — инвариант
> `webhook_secret = sha256(token)` самосогласован, ответ зовёт род отдельно
> (`secretsExternal`), экран — `danger`. Ранний 200 приёма вебхука оставлен как
> есть (протокол Telegram: повторы только на не-2xx). Пометка: метрика
> неизвестных секретов в `/health` — отдельная задача, если случаи появятся.
...
> Аудит 06.09.2026: подтверждён по коду. Три цикла живы (`routedFetch` зовут runner/orchestrator/service — шапка `llm-router.js:13–15`); сведение слоя не начиналось.
...
> Аудит 06.09.2026 (живая проверка прода): в `/opt/integram/shared/backups/` остался один каталог `uav` — архив из замера 23.08 исчез (ручная приборка?), но МЕХАНИЗМ не изменился: `INTEGRAM_BACKUP_RETENTION_DAYS` не существует, «backup» в workspace-purge — 0 вхождений. Долг актуален.
...
> Аудит 06.09.2026: закрытие подтверждено. `force: true` в bootstrap — `lazy-init.js:322`; `poolScopedMemo` и `invalidateDb` проверены в TD-041.
...
> Аудит 06.09.2026: подтверждён. Все три флага живы: `portal/router.js:4518`, `orgs/service.js:13`, `meta-kb/service.js` (`_tablesReady`); `pm`/`nightcall` по-прежнему вне BOOTSTRAP_MODULES (`lazy-init.js:281`).
...
> Аудит 06.09.2026: подтверждён по коду. `prefetchFormulaVars` пропускает EAV при совпадении — `computed-reqs.js:168` (`if (computedIds.has(reqId)) continue`); сведение двух мест не начато. Живой замер с setval не повторялся.
...
> Аудит 06.09.2026: подтверждён и УТОЧНЁН. Заслон (`guard-architecture.sh:48–58`) по-прежнему считает строки `vi.mock('../../<модуль>/…')` импортами и не исключает `__tests__`; обход через `touch` жив.

> Исполнено 19.09.2026 (TD-126): `__tests__` исключён из кроссмодульной проверки (`guard-architecture.sh:51-52`); краснота-зелёность доказана ручным stdin-JSON (тест-файл с тремя `vi.mock` — было exit=2 BLOCKED, стало exit=0; код-файл с тремя импортами — по-прежнему exit=2). Обход через `touch` ОСТАЁТСЯ известной дырой класса «заслон живёт у звавшего» — хук проверяет только новые файлы (`[ -f "$FILE" ] && exit 0`), PreToolUse не отличит Edit с большим диффом; сознательно не чинилось.
...
> Аудит 06.09.2026: подтверждён по коду (`findSecretsInText` требует присваивания `имя=значение`; `SECRET_KEY_RE` с `api[-_]?key` — `workspace-carry.js:207`). Заголовки с ключом по-прежнему срезаются целиком без плейсхолдера. Закрыто планом TD-127 (PLACEHOLDER для заголовков).

> **Принято 14.09.2026, кода не трогали.** Ограничение сказано в коде по месту (`workspace-carry.js`, комментарий «ЧЕГО ОНО НЕ ЛОВИТ И НЕ СМОЖЕТ»); отказ покрывает случай присваивания, не класс — средство обязано обещать ровно это.

**2. Тайна в заголовке уезжает без места под подстановку. — ЗАКРЫТО 20.09.2026 планом TD-127.** Запись `headers`, чьё имя отвечает `SECRET_KEY_RE`, больше не срезается целиком: `rebuildHeaders` (`workspace-templates.js`) собирает её заново по образцу ветки `auth` — имя уцелевает, значение получает `{{CONN<n>_H<i>}}` (своя серия, не общий `_TOKEN`); значение с параметровой ссылкой `{{…}}` возвращается как есть. Плейсхолдеры едут в манифесте (`headerPlaceholders` у коннектора и требования `connector_token`), экран мастера рисует по полю на каждый, `fillConnectorTokens` подставляет литеральной заменой. Незаполненный литерал валит первый прогон громко (`interpolate` strict). Остаток записи ниже оставлен как история:
...
> Аудит 06.09.2026: подтверждён. `WorkspaceList.vue` по-прежнему не читает ни `missingConnectors`, ни остальные поля (grep пуст); отказ снятия по-прежнему виден только в API/журнале.
...
> Аудит 06.09.2026: подтверждён. SELECT без `is_system` — `workspace-templates.js:2178`; починка обоих концов (поле в снятии + идемпотентность наложения) не начата.
...
> Аудит 06.09.2026: закрытие подтверждено. `valueCanBeSecret`/`COUNTER_VALUE_RE` — `workspace-carry.js:251–317`.
...
> Аудит 06.09.2026: подтверждён по коду. Точное написание условия `effectiveAuth.type === 'x'` живёт в `connectors/service.js:614+`; сторож-разбор с перечислением форм в записи. Исполнения скрипта не повторялось.
...
> Аудит 06.09.2026: подтверждён по коду. `secretsInFreeText` в снятии шаблона — `workspace-templates.js:1723`, подузлы отчётов и содержимое репо им не обходятся. Перебор 617 тысяч строк не повторялся.
...
## ~~TD-133: круг шаблона теряет значения записей — многозначная колонка и «Статус»~~ ЗАКРЫТ

**Закрыт 20.09.2026** (план `docs/superpowers/td-plans/TD-133.md`). Два слоя.

1. **Поимённая сверка значений** — `значенияЗаписей` в `scripts/template-roundtrip-check.mjs`
   + `diffValueMaps` в `scripts/lib/record-values.mjs` (юнит — `scripts/__tests__/record-values.test.js`):
   ключ «тип :: запись :: колонка» по именам, значение — величина дословно либо имя записи-цели;
   перечень многозначной ссылки — по частям. Сошедшееся множеством, но разошедшееся формой —
   справка, не провал; повторы не вычищаются («a, a» против «a» — потеря). Отбор строк — тот же
   `eavSel`, что у счётной строки «значения записей»; строка счёта осталась рядом как грубая сводка.
2. **Корень потери «Источники::MULTI:»** — извлечение (`workspace-templates.js`, прямой образец):
   многозначная ссылка лежит ОДНОЙ строкой с перечнем через запятую (`'1894, 1895, 1896'`),
   прежний код гнал её целиком в `Number()` → NaN → сырая строка в шаблон → посев искал запись
   ПО ИМЕНИ перечня и терял значение. Теперь перечень разбирается по частям, каждая резолвится
   в метку `D_R<n>`; неразрешённая часть едет сырой и досчитывается посевом в `unresolved` вслух.

Живой круг `tender_demo` → свежая копия: ДО правки diff назвал потерю поимённо
(«Тендеры :: ООО Тест :: Источники: исходная „Комус, Самсон, Wildberries", копия значения нет»),
ПОСЛЕ — «все 61 поимённо совпадают». «Статус» поимённо зелёный и до правки: доезжает обратным
образцом, потери нет (гипотеза 2б плана) — править было нечего. Счёт 61/63 после фикса — форма
хранения перечня (одна строка против трёх обратных), правит поимённый diff.
...
> Аудит 06.09.2026: подтверждён по коду. Сверка «значений записей» в `scripts/template-roundtrip-check.mjs` осталась счётной (строка `значения записей: vals.n` в отчёте, `:314`); сверки поимённо нет.
...
> Аудит 06.09.2026: закрытие подтверждено. `danglingPortalRefs` (`scripts/template-roundtrip-check.mjs:440+`) и грант-сверка с подсчётом висячих привязок (`:259+`) на месте.
...
> Аудит 06.09.2026: подтверждён по коду. Голая главная колонка в SELECT — `report-engine.js:1283–1284`, пропуск в GROUP BY — `:1634`, `MIN()` для служебных — рядом; правок с 02.09 нет. Прочие пункты (значок, `name`, одиночный multi, `block_order`) не перепроверялись в этом проходе.
...
> Аудит 06.09.2026: подтверждён. Приговор с условием пересмотра на месте (`workspace-ownership.js:146–152`); пересмотр так и не сделан.
...
> Аудит 06.09.2026: подтверждён. `!field.startsWith('req_')` — `views/service.js:607`; ключи `EQ_`/`LIKE_` по-прежнему отбрасываются.
...
**Дата:** 2026-08-23 · **Источник:** ветка `fix/template-live-roundtrip` · **Закрыт:** 2026-09-19

**1. Сверка круга не считает прав** (`_v2_grants`), хотя роли считает — раздел, который
заведомо теряется (TD-123), ею не покрыт. — **Закрыто кодом 23.08.2026** (`51f54f5f3`,
`d8bfe5642`): счёт «права ролей (переносимые)» в `scripts/template-roundtrip-check.mjs`
(запрос к `_v2_grants` по несистемным ролям, `:243-266`, строка счёта `:320`, справка
`:560-563`) сверх счёта — тождество привязок портала по именам и сравнение покрытия
правами «не хуже, чем у источника».
...
только на втором наложении. — **Закрыто 19.09.2026** (исполнение TD-138): сверка круга
судит doc-привязки по origin-карте ADR-029; подмена одноимённого документа называется
строкой «указывает на DOC<m>, а шаблон ставил DOC<n>»; копия без origin-строк — пропуск
вслух отдельной строкой.
...
чужой записи, при верных колонках. — **Закрыто 19.09.2026**: различение дала
ссылочность колонки (`col.refTypeLocalId`) плюс карта записей (`recordLocalIds` /
`recordIdMap`) — четвёртый довод `mapViewConfig` (`{ записи, refCols }`); сырой номер у
ref-колонки на наложении снимается вслух; сверка круга проверяет ref-отбор копии.
...
**Закрыт 19.09.2026 удалением писца** (`useSavedViews.js`, `ObjectList.vue`,
`VIEW_CONFIG_SPACE`); перебор по деревьям читателя не нашёл, живые null безвредны.

**5. Удвоение системных автоматизаций подтверждено заново:** 2 → 6 на замере. Следствие
TD-129, накапливается с каждым наложением. — См. TD-129.

**6. Прогон `isolated-runner.test.js` (заслон SSRF) один раз упал под нагрузкой полного

> Аудит 06.09.2026: подтверждён и объяснён TD-139 — причина падения была DNS, не порты (см. аудит TD-139). Пункты 1–5 живы: круг по-прежнему не считает `_v2_grants` (grep по `scripts/template-roundtrip-check.mjs` — файл в корне репозитория — пуст). — Исправление 19.09.2026: пометка о «пустом grep» не соответствовала дереву — гранты в файле есть с 23.08.2026 (`51f54f5f3`, `d8bfe5642`, оба на master, `git branch --contains d8bfe5642`).
...
> Аудит 06.09.2026: подтверждён по коду. `validateOutboundUrl` — `isolated-runner.js:12`, порядок проверок (dns до суффикса `.local`) — `url-guard.js:146→155`. Повторного замера таймингов DNS не делалось.
...
> Аудит 06.09.2026: подтверждён. `deleteObject`/`getObject` по-прежнему идут через службу (`tools/objects.js:40`), три остальные — собственные реализации (`:578,831,955`); сторож `objects-no-second-impl.test.js` на месте.
права не расходятся; расходится поведение.

> **Исполнено 19.09.2026 (TD-150):** `updateObject` и `createObject` сведены со
> службой (`tools/objects.js` — переходники; `listObjects` был сведён в TD-108).
> `resolveRefValue` переехал в общий `utils/eav-ref.js`; `saveRequisites` принял
> `opts.resolveRefs` (резолв ссылки именем и формой «Имя (id:N)» — только для
> пути ИИ) и возвращает фактически записанные колонки; служба отдаёт
> `data.updated`, эмитит `requisite.changed` и на ссылочные колонки (союз
> событий) и пробрасывает `source: 'ai'` из opts. Состав `changedReqIds` —
> решение **(B)**: «фактически записанные, без сравнения» для обеих дверей.
> Живое перечисление on_update-автоматизаций рабочих воркспейсов из worktree
> не выполнено, поэтому рекомендованный вариант (A) не принят: **«прислал ≠
> изменил» остаётся договором события** (`valueChanged` = «поле прислали»);
> REST-состав сузился на ключи, которые saveRequisites прежде пропускал молча
> (immutable, неизвестная колонка, NaN-ссылка). Живой прогон create/update
> с именованной ссылкой и мультиколонкой после выкладки — по плану,
> не выполнен (execSql-заглушки SQL не исполняют).
...
> Аудит 06.09.2026: подтверждён по коду. Отбор прав с этим условием — `workspace-templates.js:2353`; глушащий `.catch(() => ({ rows: [] }))` — `:595,2981`.
...
> Аудит 06.09.2026: подтверждён по коду; замеров «0 из 632» не повторялось.
...
> Аудит 06.09.2026: подтверждён. Сужение на месте — `workspace-templates.js:2434` (`.catch` рвёт на всём, кроме `42P01`), а теста, роняющего снятие на ином коде, по-прежнему нет.
...
> Аудит 06.09.2026: подтверждён по коду. Порог жив в виде `ok: live > 0 && confirmed * 2 > live` — `workspaces/template-origin.js:911`; признак противоречия (вместо подтверждения) не заведён.
...
> Аудит 06.09.2026: закрытие подтверждено. `resolveLookupType` — `lookups/service.js:24`.
...
> Аудит 06.09.2026: подтверждён по коду. `viaColumnIds` принимается со сверкой только вида `/^C\d+$/` (`template-blocks.js:109–112`); проверка выводом из данных не сделана.
...
> Аудит 06.09.2026: подтверждён по коду. `refsClearedSample` ограничен десятью — `template-blocks.js:487`; асимметрия с неограниченным `multiRefTrimmed` на месте.
...
> Исполнение 20.09.2026 (план `td-plans/TD-156.md`): п. 1 — амендмент ADR-029
> «Восстановление снятой связи»; п. 2 — `restoreDroppedRefs` в
> `template-blocks.js` + вызов в `applyTemplateToWorkspace` (`refsRestored`)
> + фолбэк разрешения по карте в `_seedDemoData`; п. 3 — `multiRefTrimmed`
> счёт + `multiRefTrimmedSample`; п. 4 — сверка круга печатает снятое и
> предел; п. 5 — `selection.limitReached` на всех трёх циклах; п. 6 —
> чекбоксы блоков в `WizardStepPreview.vue` (умный путь мастера), `blocks`
> в MCP `create_workspace` и в описании `apply_workspace_template`. Осталось:
> п. 6 частично — прямой путь `WizardStepManual.vue` (создание из шаблона без
> шага предпросмотра) блоков не спрашивает; п. 7 — ручная пересъёмка эталона.
...
> Аудит 06.09.2026: подтверждён живыми числами. `#4a7c59` — `frontend/src/composables/usePortalEditor.js:13`; `#6B7C3E` — `portal/composables/usePortalTheme.js:47`. Расхождение на месте, сторожа нет.
...
> Аудит 06.09.2026: подтверждён по коду. Дословное сравнение FILTER — `row-permissions.js:235` (`String(actualVal) !== String(rule.config.value)`); «опубликовано» — данные trytofly.
...
> Аудит 06.09.2026: подтверждён. Фильтр `!c.hidden` — `ReportView.vue:392`; выставляющего UI по-прежнему нет.
...
> Аудит 06.09.2026: подтверждён по коду. Страничный обход с подсчётом продвижения по номерам — `portal-anon-exposure.mjs:130,140`; оставшиеся четыре пункта (неразогнанная ветка следов, `parentScope`, узкие образцы, раскладка) — сознательные ограничения, не изменились.
...
> Аудит 06.09.2026: подтверждён с уточнением структуры: ensure теперь идёт через `portal/teamchat/router.js:11` → `teamchat/service.js` `ensureTables` (`CREATE TABLE IF NOT EXISTS`, :95–157) — обработки `42P07`/advisory по-прежнему НЕТ; повторное замерение гонки не делалось.
...
> Аудит 06.09.2026: закрытие подтверждено. tildacdn в CSP — `portal/server/middleware/csp.ts:34–35`.
...
> Аудит 06.09.2026: подтверждён как правило-запись (сам инцидент 29.08 воспроизводить не нужно). Процедурного заслона (чек-лист в deploy-доке или скрипт сверки) не появлялось — правило живёт только здесь.
...
> Аудит 06.09.2026: подтверждён по коду. Оба вызова `toast.error(new Error(…))` живы — `usePortalEditor.js:185,189`.
...
> Аудит 06.09.2026: закрытие подтверждено. `GET /api/kit/versions` — `portal/router.js:3294`; `kit_widget` — `moduleRegistry.js:50`.
...
> Аудит 06.09.2026: подтверждён. `EditorRightPanel.vue:7` всё ещё ввозит `TEMPLATE_PRESETS` из `@/composables/usePortalEditor` (фронтендовая таблица с `#4a7c59`); `EditorPreview.vue:27` — из `@portal/composables/usePortalTheme`. Расхождение живо.
...
## ~~TD-165: остаточная гонка acceptInvitation — членство вставляется до guarded UPDATE~~ ИСПРАВЛЕН

**Дата:** 2026-08-31 · **Источник:** ревью ветки pm-p0-orgs (fix ca9e07f61)

**Исправлен 19.09.2026** (план `docs/superpowers/td-plans/TD-165.md`): acceptInvitation — вся цепочка (lookup с `FOR UPDATE OF i`, expiry, email-проверка, INSERT членства, guarded UPDATE) внутри `withTransaction`; revoke — зеркальный guard `AND status = 'pending'` + отказ 404 при `rowCount = 0` (без него отзыв перезаписывал accepted → revoked). Тесты — `workspaces/__tests__/invitation-pending-guard.test.js` (7, красные до правки), живая проверка `FOR UPDATE OF i` на join-запросе при исполнении НЕ выполнялась (worktree-исполнение без дев-базы; суффикс обязателен в форме `OF i` — голый `FOR UPDATE` на LEFT JOIN даёт ошибку планировщика, план это фиксирует как риск шага живой проверки).

**Поправка к пункту про сортировку:** запись ошибочна — комментарий в `orgs/__tests__/pm-issue-order.test.js:79` ВЕРЕН («дала бы порядок 4,7,5,6,3,1,2»), а заявленный здесь «реальный порядок» `[2,1,3,6,5,6,5,7,4]` — 9 элементов с повторами для перечня из 7 дней и ничему не соответствует. Доказательство: `node -e "const d=x=>new Date(2026,8,x,12); console.log([7,1,5,3,6,2,4].sort((a,b)=>String(d(a)).localeCompare(String(d(b)))).join(','))"` → `4,7,5,6,3,1,2`. Править нечего.

> Аудит 06.09.2026: подтверждён по коду. INSERT членства (`workspace-invitations.js:140`) по-прежнему идёт ДО guarded UPDATE статуса, вне транзакции и без `FOR UPDATE` — приём приглашения и отзыв не сериализованы.
...
> Аудит 06.09.2026: подтверждён по коду. Из четырёх пунктов лечения не сделан ни один (описания confusable-пар, редирект-подсказки, ужатие instructions — grep по признакам пуст); дисциплина «<80 имён» записана в память. Частично перекрыт устройством TD-168-фикса (activation-memory, 0.7.36) — восстановление активации по воркспейсу убрало часть срывов.
...
> ЗАКРЫТО 17.09.2026 — код закрыт устройством 0.7.36 (activation-memory.js, план TD-168.md — запись закрытия). Прогон mcp-тестов зелёный.
...
> Аудит 06.09.2026: закрыт устройством 0.7.36 (`activation-memory.js`, коммит 8f9f929ec; готча `[mcp/субагент-и-активация]` фиксирует восстановление активации по воркспейсу). Формулировка записи про «вызов маршрутизируется в чужой инструмент» относится к клиенту БЕЗ этого фикса — при живом fix повтор замера не воспроизводился. ~~Открыто~~ → **закрыто 0.7.36**, хвост TD-167 остаётся.
...
> Аудит 06.09.2026: подтверждён по коду. Заслон (гашение 42P07/42710) жив в другом месте — `pm/schema.js:262`; воспроизведение гонки и advisory-заслон не сделаны.
...
> Аудит 06.09.2026: подтверждён по коду. `GET /pm/board` по-прежнему принимает только sprintId/boardId/showAll (`pm/router.js:386–395`); labels в getBoard нет. В списке серверный фильтр жив (`pm/service.js:425` `i.labels && ?::text[]`).
...
> Аудит 06.09.2026: подтверждён с уточнением: ListView больше не шлёт limit:50 — зовёт `pm.loadIssues` без лимита (`ListView.vue:119`), страницу режет серверный дефолт; дерево по-прежнему из одной страницы.
...
> Аудит 06.09.2026: подтверждён. Двухместность на месте: `KIT_VERSION = '0.9.30'` (`portal-kit/src/index.js:12`) и `version: 0.9.30` (`package.json:3`) — совпадают дисциплиной бампа, инжекции нет.
...
> Аудит 06.09.2026: подтверждён по коду. «Начало» — по-прежнему read-only при непустом значении (`IssueDetail.vue:228`).
...
> Аудит 06.09.2026: подтверждён. В `.github/workflows/` — только sync-docs.yml и sync-mcp-server.yml; тестового CI по-прежнему нет.
...
> Аудит 06.09.2026: подтверждён по коду. `cmd_health` проверяет только `/api/v2/health` (`deploy-prod.sh:453`); проб модулей нет.
...
> Аудит 06.09.2026: ЧАСТИЧНО закрыто. `disable-model-invocation` убран из SKILL.md (теперь `allowed-tools`); финальный шаг plan-to-pm добавлен в subagent-driven-development (`SKILL.md:28`); executing-plans и обобщение session-to-trytofly — не сделаны.
...
> Аудит 06.09.2026: подтверждён. `scripts/new-worktree.sh` не существует; pnpm-источник не найден.
...
> Аудит 06.09.2026 (живой замер): `max(created_at)` в `usadba._v2_presence_log` → `2026-08-20 23:13:46` — тишина 17 дней, канал мёртв.
...
> Аудит 06.09.2026: подтверждён по коду. Константы таблиц живут в `orgs/pm-aggregation.js:71–72`; экспорта из `pm/schema.js` нет.

**Приоритет:** низкий — расхождение проявится только при переименовании в PM.

---

## ~~TD-180: порядок my-issues живёт в двух местах — compareIssues и SQL ORDER BY~~ ЗАКРЫТ
...
> Аудит 06.09.2026: подтверждён по коду. `compareIssues` — `orgs/pm-order.js:37`; дублирование рангов не сведено. *(Помета ошибочна: ранги уже сведены коммитом `9d6de35f8` от 03.09.2026 — `PRIORITY_RANK` + `sqlPriorityCase()` в `pm-order.js`.)*

**Закрыт 21.09.2026:** ранги — `9d6de35f8` (`sqlPriorityCase()`); вторичные ключи (`updated_at DESC, number ASC`) — `issuesOrderBy()` в `pm-order.js`, литерал в `pm-aggregation.js` заменён; сторож «рукописный ORDER BY задач в orgs не остался» + golden-пин + юнит-сверка компаратора — `pm-issue-order.test.js` (краснота доказана до правки). Курсор (`cursorSql`) не переписывался — его защищает пин: правка порядка краснеет и заставит обновить курсор руками. *(ПОПРАВКА 21.09: прежняя помета «Закрыт 19.09» была ложной — коммита с `issuesOrderBy` в backend не существовало, закрыт только docs-коммит e02e298d8; код исполнен по плану 21.09.)*
...
> Аудит 06.09.2026: не перепроверялся в UI — объёмная фича, признаков реализации в этом проходе не искали.
...
> ЗАКРЫТО 17.09.2026 — реализовано iam/service.js:858 (приглашения переписываются на новый адрес, сверено ревью). План TD-182.md — фиксация.
...
> Аудит 06.09.2026: подтверждён по коду; resend/join по user_id не реализованы.
...
> Аудит 06.09.2026: подтверждён — GeoIP не появлялся.

---

## ~~TD-184: Баннер «письмо отправлено» смены email живёт только в компоненте~~

> ЗАКРЫТО 17.09.2026 — баннер переживает перезагрузку: Profile.vue зовёт iamGetPendingEmailChange (сверено ревью). План TD-184.md — фиксация.
...
> Аудит 06.09.2026: ЧАСТИЧНО закрыто. Бэкенд жив: `GET /iam/me/pending-email` (`iam/router.js:191`) и `getPendingEmailChange` (`services/iam.js:21`); `Profile.vue` его НЕ зовёт — постоянного баннера нет.
...
> Аудит 06.09.2026: подтверждён по коду. Прямая проводка `due_date` без нормализации — `tools/pm.js:61`; служба (`pm/service.js:229`) строку «null» не отличает от даты. Живого повтора не делалось.
...
> Аудит 06.09.2026: подтверждён по коду. `portalAuth('*')` на pm-чтениях жив (`portal/router.js:3334,3351,3371`); org-портал не заведён (организация по-прежнему одна).
...
> Аудит 06.09.2026: закрытие подтверждено. Тест `llm-router-refusals.test.js` существует (`ai/__tests__/`); открытый хвост (видимость конфигурации цепочки владельцу) не закрыт.
...
> Аудит 06.09.2026: подтверждён по коду зеркала: `mcp-server/mirror/publish.yml:104` — `sleep 10`, окно опроса не увеличено. Правка живёт в зеркале руками — не перенесена.
...
> ЗАКРЫТО 17.09.2026 — typeId берётся из employeeTable-конфига, сторож U32 (staff-session.test.js) зелёный на текущем дереве.
...
> Аудит 06.09.2026: подтверждён. `WHERE o.t = 313` — `portal/router.js:2348`.
...
> Аудит 06.09.2026: подтверждён. Мёртвая деструктуризация `({ meta, ...rest }) => rest` — `tools/presentations.js:18`; «presentations» в `mcp-server/index.js` — 0 вхождений.
>
> ~~Мёртвая деструктуризация~~ **ОПРОВЕРГНУТО 17.09.2026 (ревью TD-192): деструктуризация ЖИВАЯ, не дефект.** `svc.list` выбирает `SELECT p.*` (`presentations/service.js:67`), а колонка `meta JSONB` существует в `_v2_presentations` (`presentations/schema.js:39`) — вырезание реально скрывает служебное поле из ответа `pres_list`. Сносить нельзя: в ответ инструмента попал бы JSONB `meta`. Это единственное место вырезания в модуле — сделано намеренно. Запись закрыта как «не дефект», правки кода не требуется. Вторая половина (группа presentations в буллетах MCP) закрыта ранее — `mcp-server/index.js:294`.

## ~~TD-193: мастер красный — invoke-many и tool-counts-sync после a7817ea49 (write-агент)~~

> ЗАКРЫТО 17.09.2026 — оба файла зелёные на текущем дереве (полный прогон 871 файл / 8334 теста, 17.09). Фикс-пакет влился PR-волной.
...
> ЗАКРЫТО 17.09.2026 — MCP-инструменты истории конфига есть (a6df2eb2b: list_portal_config_history + restore_portal_config с HITL).
...
## ~~TD-189: Портальный маршрут удаления документа не существует — delta-документ с портала удалить нечем~~ ЗАКРЫТ

> ЗАКРЫТО 17.09.2026 — план `docs/superpowers/td-plans/TD-189b.md` исполнен: DELETE /api/documents/:docId в `portal/router.js` (requireStaffPortalJwt + portalGrantsMiddleware + rateLimit + staffCanWriteDocs → soft delete через `documents.remove`), кнопка двухшагового удаления в portal-kit DocumentEditor 0.9.32 (emit `doc-delete`), 5 новых кейсов в document-routes.test.js. Follow-up: кнопка в fst-portal `Docs.vue` (codespace-репо, отдельная выкладка).

`DELETE /:db/portal/api/documents/:id` отсутствует (404) — в portal/router.js есть создание (POST /documents), правка заголовка (PATCH, сужен до {title}) и операции с БЛОКАМИ, но удаления документа целиком нет. Плюс в Docs.vue (fst-portal) кнопка удаления не видна. Последствие (замер 05.09.2026): документ «Проверка write-агента» (id 57), созданный агентом чата в проверке write-агента, остался в воркспейсе навсегда — убрать его нечем ни с портала, ни инструментами (read_blob/patch на документы не действуют). Лечение: маршрут DELETE (staff, гранты как у блоков) + кнопка двухшагового удаления в DocumentEditor/Docs.vue.

> Аудит 06.09.2026: подтверждён по коду. В `portal/router.js` есть только DELETE блока (`:2146`); удаления документа целиком по-прежнему нет.

## TD-190: Делегированный portal-agent не имеет PM-инструментов и generic EAV-чтения

При unrestricted-делегировании (staff портала с allowWrite, фикс 036fa4488) оркестратор передаёт задачи встроенным под-агентам. Замер 05.09.2026: у `portal-agent` нет create_issue (честный отказ на «создай задачу PM») и generic EAV-чтения (подсчёт записей произвольной таблицы невыполним), у `tables-agent` чтение есть. Лечение: расширить тулсет portal-agent (create_issue/list_objects) ИЛИ научить оркестратор не делегировать такие задачи, а исполнять самому (у оркестратора оба инструмента есть). Отдельно: под-агент иногда отвечает «готово» на неуспешный вызов инструмента — корень (тихий пропуск неизвестного поля в planRequisiteWrites) исправлен громким отказом (коммит с тестом unknown-field-loud-error), но общая дисциплина «не рапортовать успех до проверки результата» остаётся за промптами под-агентов.

> Аудит 06.09.2026: подтверждён по коду. Тулсет `portal-agent` (`agents/portal.js`) по-прежнему без `create_issue`/`list_objects` (grep пуст).

<<<<<<< ours
## ~~TD-195: 10 старых манифестов шаблонов — автоматизации без localId; повторное применение дублирует их~~

> ЗАКРЫТО 17.09.2026 — сторож ужесточён, тесты зелёные (77 passed). Остаточная ручная чистка манифестов — за владельцем.

=======
> ЗАКРЫТО 19.09.2026 — план `docs/superpowers/td-plans/TD-190b.md` исполнен: read-половина закрыта 08.09.2026 коммитом `733457742` (`list_tables`/`get_table_schema`/`list_objects`); write-половина — `pm_create_issue` в тулсете portal-agent (def из TOOL_DEFS, group `portal`), строка «Задачи PM» в systemPrompt (не писать задачу в EAV-таблицу напрямую) и пункт дисциплины «не рапортовать "готово" до успешного ответа инструмента». Misroute-митигации: PM-примеры в AGENT_EXAMPLES не добавлялись, meta.description без PM-лексики. Граница специализации закреплена тестом portal-agent-read-tools.test.js (create/update/delete_object, pm_delete_issue, pm_update_issue — not.toHaveProperty); счётчики обновлены sync-tool-counts.mjs (portal 37→38 в ai.md).

## TD-195: 10 старых манифестов шаблонов — автоматизации без localId; повторное применение дублирует их
>>>>>>> theirs

`beekeeping`, `ecommerce`, `education`, `farmshop`, `freelance`, `manufacturing`, `meta-kb-archipelago` (4), `perelidos-accelerator` (4), `real-estate`, `restaurant`, `services-crm` — итого 25 автоматизаций в 10 манифестах `backend/src/data/templates/workspaces/` без `localId`. Применение апсертит по localId (`workspace-templates.js:3432`, `было = a.localId ? origin.get(...) : undefined`) — без него повторное применение молча INSERT-ит копии автоматизаций. Новый extraction присваивает `A<n>` всегда (строка ~2291), эти файлы его опередили. Сторож `manifests.test.js` (0a0807208) проверяет уникальность среди присутствующих, присутствие намеренно не требует — иначе каталог краснеет целиком. Лечение: раздать `localId` (`A1`, `A2`, …) всем автоматизациям десяти манифестов, после чего ужесточить сторож до обязательного присутствия.

> Замер 08.09.2026: прогон сторожа с требованием присутствия даёт 11 красных; перечень манифестов — из отчёта исполнения плана `docs/superpowers/plans/2026-09-08-sppr-pr151-remediation.md`.

## ~~TD-196: мастер красный — 4 падения из чужих коммитов (не sppr)~~

> ЗАКРЫТО 17.09.2026 — все 4 падения зелёные на текущем дереве (полный прогон 17.09).


Полный прогон 08.09.2026 (коммит 8310607cc): 7156 passed, 4 failed, все четыре — не из ветки sppr-ремедиации: (1) `mock-paths-resolve` — коммит 0d8b0e34c добавил тест `unknown-field-loud-error.test.js` с vi.mock пути `utils/column-validation.js`, которого в репозитории нет; (2) `invoke-many` — счётчик `portalAllowedAgentSlugs(` меньше ожидаемого после 9aecb79f2/d3c4cc7ea; (3) `tool-counts-sync` — числа в ai.md/README/AI_AGENT_GUIDE отстали от кода (лечится `node backend/scripts/sync-tool-counts.mjs`); (4) `tool-description-sync` — описание `portal.js::update_portal_module` разошлось с TOOL_DEFS. Чинить владельцу сессиям-авторам; до починки полный прогон не бывает зелёным.

> Закрыт 08.09.2026: localId `A1..An` выданы всем 25 автоматизациям десяти манифестов (семантика проверена глубинным сравнением с HEAD), сторож ужесточён до обязательности метки (290 тестов зелёные).

## TD-197: role-routing для фонда — настройка заблокирована интервью; срок неизвестен

Код role-routing (`assigneeRole`/`assigneeStrategy` в `create_issue`) готов и протестирован (08.09.2026, коммиты 6a8b53cab…b565e3340, PM ai2o #205: 5/6). Остался замкнутый на людей хвост — PM-211 «Настройка фонда»: завести в fst EAV-таблицы-роли (Аналитики/Юристы/Финансисты/Директора), заполнить участниками (логин в колонке «Логин»), создать автоматизацию «Заявка → 4 задачи по ролям» (полный JSON в плане 2026-09-07-role-routing-issues.md, Task 6) и проверить действием. Блокер: интервью сотрудников фонда (добро Арсения Розинского дано 05.09.2026, дата неизвестна) — без них неизвестны имена ролей и состав участников; 4 из 11 портфельных компаний тоже ждут данных фонда. Не забывать: при следующем контакте с фондом запросить заодно список компаний и файл презентации для реестра (fst t=133).

---

## ~~TD-198: doc-processor reconcile — 18 legacy-воркспейсов без `processing_status` шумят логами и невидимы для сверки~~ ЗАКРЫТ

**Закрыт 10.09.2026.** `reconcileWorkspacePending` в `doc-processor.js`: запрос переведён на `fatal: true`, классы отказа разобраны по pg-коду (`err.cause.code`): нет колонки (42703) — `ensureFilesTable` + повтор запроса, отказ лечения — warn однократно с тихим ретраем в каждом цикле, прочие отказы — warn один раз на `db:код`. Нет таблицы — прежнее молчание (execSql возвращает DB_NOT_FOUND без throw). После первого цикла на проде колонки появляются у всех 18 legacy-таблиц и шум обрывается.

**Файлы:**
- `backend/src/api/v2/modules/files/doc-processor.js:80-92` — reconcile-обход всех воркспейсов
- `backend/src/api/v2/modules/files/service.js:354-411` — `ensureFilesTable` (ленивые идемпотентные ALTER)

**Проблема:**
Reconcile каждые 3 минуты делает по всем воркспейсам сырой `SELECT 1 FROM "<db>"._v2_files WHERE processing_status = 'pending'` БЕЗ `ensureFilesTable`. У 18 из 125 воркспейсов прода таблица `_v2_files` заведена старым DDL без Phase-10 колонок, а идемпотентные ALTER выполняются лениво — только когда какой-нибудь files-вызов трогает воркспейс; у пустых/неиспользуемых он не срабатывает никогда. Каждый цикл такие воркспейсы дают warn `Couldn't execute query [doc_proc_reconcile_pending] column "processing_status" does not exist` (~8 600 строк шума в сутки), catch при этом комментарием «table may not exist» описывает не тот случай. Побочный эффект: reconcile эти воркспейсы не видит — функционально безвредно (новый код не может писать туда строки без ensure, старые строки статуса не имеют), но защита от пропущенных pending-файлов на этих 18 не работает.

> Аудит 09.09.2026: counts из information_schema прода — 125 воркспейсов, таблица есть у всех, колонки `processing_status` нет у 18 (a2025, abc123, alekseymavai, alex, asmo_seo_ya_ru, beecrmtest, beelog, clear, cryptoanalitik, devteam, gramme, kuku, revers, revers_atr, test, test1, testovyy, vork1); тот же список — в warn'ах лога. В более старых лог-файлах по 475 warn'ов на файл — шум не новый.

**Решение:**
Самолечение: в reconcile-ветке отказа однократно звать `ensureFilesTable(pool, db)` и повторять запрос — после первого цикла все 18 таблиц получают колонки и шум обрывается; альтернатива — разовый миграционный прогон `ensureFilesTable` по всем воркспейсам при старте воркера.

**Приоритет:** Низкий — шум в логах, не отказ; чинить одной правкой в catch.

---

## TD-199: ai/agent/tools/excel.js — последняя копия последовательности «запись + recordFileMetadata»

**Файлы:**
- `backend/src/api/v2/modules/ai/agent/tools/excel.js:37`

**Проблема:**
После перевода пяти HTTP-маршрутов на `makeDiskUpload` + `storeFile` (09.09.2026, files.md «Storing a file from another module») единственное место, повторяющее последовательность записи файла своими руками, — Excel-инструмент агента. Пишет buffer молча поверх существующего имени (без `uniqueFilename`), отдельно зовёт `recordFileMetadata`.

**Решение:**
Перейти на `storeFile`/`storeFileBuffer` из files/service — единый контракт (no-overwrite, MIME-валидация, очередь обработки).

**Приоритет:** Низкий — инструмент один, поведение согласовано с ним самим.

---

## TD-200: temp-сироты в /tmp/integram-uploads — уборщика нет

**Файлы:**
- `backend/src/api/v2/modules/files/upload-middleware.js` — TMP_ROOT
- `backend/src/api/v2/modules/ai/router.js` (POST /upload — обёртка multer)

**Проблема:**
При `kill -9`/краше процесса посреди загрузки недописанный temp-файл (до 500 MB) остаётся в `os.tmpdir()/integram-uploads` навсегда — multer 2.x чистит только штатные аварийные пути. Второй, ограниченный класс: multer отработал успешно, а отклонил последующий middleware (requireRole/csrf в ai/upload) — `cleanupUploadTmp` в этой ветке не зовётся.

**Решение:**
Сweep при старте воркера: удалять файлы старше N часов из TMP_ROOT (mtime), либо TTL-обход раз в час; плюс звать `cleanupUploadTmp(req.file, req)` в error-handler'е верхнего уровня по признаку `req._uploadTmpPath`.

**Приоритет:** Низкий — диск 77 GB, эпизод редкий; фиксировать, пока не заметен.

---

## TD-201: TOCTOU в uniqueFilename → rename — параллельные загрузки одного имени могут затереть друг друга

**Файлы:**
- `backend/src/api/v2/modules/files/service.js:1190` (`uniqueFilename` — existsSync-проверка) и `:1331` (`moveUploaded` → renameSync)
- `backend/src/api/v2/modules/files/service.js:1251-1255` — `storeFileBuffer` с той же гонкой

**Проблема:**
Проверка существования имени и переименование не атомарны: два параллельных `storeFile`/`storeFileBuffer` с одинаковым именем в один каталог — второй rename молча перезапишет байты первого, его строка метаданных начнёт описывать чужие байты. Окно — миллисекунды, требуется совпадение db+каталог+имя. Унаследовано от `storeFileBuffer`, перевод на diskStorage гонку не изменил.

**Решение:**
Атомарное «rename без перезаписи»: писать во временное имя и `link()` + `unlink()` (падает на существующем) вместо existsSync-цикла, либо ловить уникальность на уровне DDL.

**Приоритет:** Низкий — вероятность ничтожна; чинить только при реальном прецеденте.

---

## TD-202: 85 объектов на проде хранят ref одновременно в обоих паттернах

**Файлы/данные:** uav (колонки 322, 41553–41560 — 76 объектов), kval (1255), perelidoz (661, 676), trytofly (330), mai_aviation (322, 41560). Выявлено `backend/scripts/audit-ref-patterns.mjs` (issue #167, 10.09.2026, exit code 1).

**Проблема:**
Запись ref-колонки обязана оставлять один паттерн (инвертированный — норма, прямой — наследие). Двойные строки означают либо рассинхрон двух представлений одной ссылки, либо мусор. Читатели (pivot, отчёты, listObjects) теперь дедупируют с приоритетом инвертированного, но данные остаются заражёнными: обе строки расходуют место и могут расходиться значениями.

**Решение:**
Бэкфилл: для каждого (объект, колонка) сверить значения обоих паттернов; при согласии удалить прямую строку, при расхождении — вывести в отчёт на решение. После — прогнать аудит-скрипт, ожидание exit 0.

**Приоритет:** Низкий — читатели защищены дедупом; чинить при следующем заходе в uav.

---

## TD-203: пост-сортировка отчёта по computed-колонке молчит об усечении на клиенте

**Файлы:** `backend/src/api/v2/utils/report-engine.js` (postSort-блок, `MAX_POSTSORT_ROWS = 5000`).

**Проблема:**
Отчёт, отсортированный по computed-колонке, на наборах >5000 строк отдаёт только первые 5000 (warning только в серверном логе), но `results.count` рапортует полный размер — клиент показывает страницы, которые невозможно открыть. Аналогичный класс: [tools/export-молчаливое-усечение].

**Решение:**
Отдавать `truncated: true` в ответе рана при упоре в потолок + пометка в UI отчётов.

**Приоритет:** Низкий — порог 5000 строк; живые отчёты с computed-сортировкой пока малы.

---

## TD-204: пост-сортировка computed — несмешиваемые типы и второй ключ сортировки

**Файлы:** `backend/src/api/v2/utils/report-engine.js` (компаратор пост-сортировки).

**Проблема:**
1. Компаратор «числа числово, остальное лексикографически» нетранзитивен на колонке, где числовые строки смешаны с начинающимися с цифры нечисловыми (`9 < 10`, `10 < '8a'`, но `9 > '8a'`). Целостный ключ: `[isNum?0:1, значение]`.
2. При нескольких ключах сортировки, где первый — computed, остальные отбрасываются (зафиксировано комментарием, до фикса был 500).
3. Даты из computed-FORMULA сортируются лексикографически — в SQL-пути это уже решено `dateSortSql`, в JS-компараторе аналога нет.

**Решение:**
Классовый ключ сортировки + dateSort-аналог для JS (dateSortSql разбирает те же три формы дат) + комбинированные ключи.

**Приоритет:** Низкий — путь ранее падал 500-й, сейчас корректен на однородных колонках.

---

## TD-205: у старых воркспейсов нет сателлита _v2_column_ai_config, маршруты column-config не делают ленивую инициализацию

**Файлы:** `backend/src/api/v2/modules/ai/service.js` (initTable, :359), `backend/src/api/v2/modules/ai/router.js:727,733` (GET/PUT /ai/column-config), `backend/src/api/v2/registry/lazy-init.js:205`.

**Проблема:**
`_v2_column_ai_config` заводится только через `ensureSideTables(pool, db, 'ai')` при создании/клоне/переносе воркспейса. Воркспейсы, созданные до появления сателлита, его не получают, а маршруты чтения/записи конфига AI-кнопок не вызывают `ensureSideTables` — configure_ai_button на них падает `relation ... does not exist`, затем (после ручного CREATE) `permission denied`, потому что ручная таблица не наследует гранты роли приложения. Замер 13.09.2026 на проде: из ~140 схем-воркспейсов у ~30 (все старые: beecrm, testovyy, kval, devteam, cryptoanalitik и др.) таблицы нет; fst получил её вручную тем же DDL, что initTable + GRANT роли integram.

**Решение:**
1. В GET/PUT `/ai/column-config/:typeId/:reqId` обернуть вызовы в `ensureSideTables(req.pool, req.db, 'ai')` в try/catch (образец — `documents/router.js:236-243`, грабля `backend/ленивые-миграции`).
2. Однократный бутстрап: прогнать initTable по всем схемам prod/integram, где таблицы нет, и выдать гранты роли приложения (или `ALTER DEFAULT PRIVILEGES`).
3. Тот же аудит для прочих ai-сателлитов того же периода.

**Приоритет:** Средний — AI-кнопки на любом старом воркспейсе не настраиваются; обход ручной, но нужен каждой базе.

**Решено 14.09.2026 (PM-283):** заслон ленивого заведения поставлен в `getColumnConfig`/`saveColumnConfig` сервиса ai — один на REST-маршруты и MCP-инструменты (`ensureAiConfigTable` → `ensureSideTables(pool, db, 'ai')`, динамический импорт против цикла с lazy-init). Одноразовый бутстрап — `backend/scripts/backfill-column-config-tables.js` (холостой прогон / `--apply` / `--only=`), заводит модульным DDL и безусловно грантует `integram` также `_v2_column_http_config` и `_v2_column_script_config` (fst-кейс). Аудит прочих ai-сателлитов: http/script-кнопки самообеспечиваются (`initTable` в каждом getConfig/saveConfig) — класса нет; таблицы `agent/history.js` лежат в общей базе и per-workspace не являются.

---
## TD-206: аватар «?» в ячейке отчёта по колонке «Пользователь» (1010), если у пользователя пустое имя

**Файлы:** `frontend/src/components/datatable/CellRenderer.vue` (ветка COLLABORATOR, userOpt/userName), `backend/src/api/v2/utils/report-engine.js` (user-колонка, JOIN `_v2_users`).

**Проблема:**
В отчётах бэкенд резолвит имя по `_v2_users.name`; фронтовый грид отчёта не грузит опции участников, поэтому аватар-инициалы считаются из пришедшей строки. Если у пользователя пустое `name` и нет `avatar_url`, ячейка показывает «?» вместо инициалов. Практически недостижимо: `name` не NOT NULL, но регистрация всегда его заполняет; «бывшие участники» показываются своим последним именем.

**Решение:**
Если когда-нибудь понадобится строгий контракт — прокинуть в грид отчёта опции участников (как в сетке таблиц, `ObjectList.loadLookups`) и брать инициалы из них. Сейчас осознанно не делается: фикс «инициалы из имени» (CellRenderer, 4ac91a58c-родословная) закрывает все реальные случаи.

**Приоритет:** Низкий — косметика на вырожденных данных.

---

---
## TD-207: медиа-ритуалы фонда — конфиги автоматизаций и вторые итерации

**Файлы:** конфиги — воркспейс `fund-demo` (MCP `create_automation`); код действий уже на проде (PM-483, PM ai2o: `send_telegram_media`, `create_document`, `run_video_job`).

**Проблема:**
Ритуалы «Корпоративная культура» v0.4 (раздел 2.2) не запущены: в fund-demo нет Telegram-бота, конфигов автоматизаций нет. Кодовая база готова и выложена.

**Что осталось:**
1. Бот фонда (токен от @BotFather — вход владельца) + синк команд.
2. Конфиги по ранбуку `docs/superpowers/plans/2026-09-15-media-rituals-runbook.md` Tasks 3–5: «Утренний брифинг» (cron 30 5 * * 1-5 UTC), `/protocol` (агент → create_document → request_approval approver:dima → create_issue), `/recap` (агент → run_video_job → send_telegram_media video; в сценарии ОБЯЗАТЕЛЬНЫ логин-шаги — рекордер без сессии, worker.js:63).
3. Живые прогоны: первый `/protocol` проверяет owner-резолв документа (scheduler-контекст без user.id), первый `/recap` — рендер и доставку видео.
4. Итерация 2: персональный брифинг каждому (per-user chatId), длинные записи → транскрипт в `/protocol` (telegram_download_file → extracted_text), агент-хранитель + «герой недели» (по журналу), наполнение glossary.vue словарём культуры v0.4 (2.3), метрики раздела 5 в journal решений (teamchat decisions).

**Приоритет:** Высокий — вечерняя презентация корпкультуры и встреча 24-го упираются в живые ритуалы; блокер один — токен бота.

---
## TD-208: undo-стеки фронтенда — путь серверного сбоя посреди undo/redo

**Файлы:** `frontend/src/composables/useObjectUndoRedo.js`, `frontend/src/composables/useReportUndo.js` (undo/redo-композаблы плана 2026-09-16).

**Проблема:**
`handleUndo`/`handleRedo` достают запись из стека ДО попытки серверного вызова. Если вызов падает (сеть/права), запись уже уехала: в redoStack лежит операция, которая не была отменена, и Ctrl+Y повторит её поверх неоткаченного состояния (например, дубль колонки после неудачного undo col-add). Паттерн одинаков в обоих композаблах; локальный порядок col-reorder применяется до серверного вызова — при сбое расходится с сервером.

**Что осталось:**
1. При ошибке возвращать запись в исходный стек (или ретраить) — единая обёртка в `useUndoRedo` вместо копипасты в каждом case.
2. col-reorder: применять серверный вызов до локальной мутации или откатывать локальную при сбое.

**Приоритет:** Низкий — edge-путь (нужен отказ сервера ровно посреди undo), happy path проверен; обнаружено ревью 16.09.2026 (планы undo: table-undo-fixes, report-edit-undo).

---
## TD-209: портальные ensure-точки — CREATE TABLE IF NOT EXISTS без обработки 42P07 (остаток TD-159)

**Файлы:** `backend/src/api/v2/modules/portal/service.js:64,101,332`, `portal/analytics.js:16` (стартовые, зовутся из `index.js:228,243`), `portal/chat/schema.js:14,27` (по запросу), `portal/account-service.js:9`, `portal/telegram-broadcast.js:19`, `portal/router.js:4187`.

**Проблема:**
Тот же класс гонки, что TD-159 (закрыт tcDdl/mkDdl с ловлей 42P07/42710/23505 по коду + recheck `to_regclass`): на свежей базе параллельный первый хит у одного из клиентов даёт 500 «already exists»/«does not exist». Приём TD-159 переносится как есть.

**Что осталось:**
1. Обернуть перечисленные DDL-точки обёрткой образца `tcDdl` (`teamchat/service.js`) / `mkDdl` (`meta-kb/service.js`).
2. Отдельный дефект стартовых точек: `.catch(() => {})` глотает ВСЁ, включая реальные отказы DDL — область молча живёт без таблиц.
3. Если класс всплывёт в третьем модуле — вернуться к варианту регистрации в `ensureSideTables` (переключатель `runModuleDdl`, xact-замок), отклонённому в TD-159 как избыточный для двух точек.

**Приоритет:** Низкий — воспроизводится только на свежей базе/реставрации дампа первым параллельным хитом.

---

## TD-210: meetings intake — мёртвая ветка записи участника в строку сообщения

`meetings/templates.js:87` — `if (pid && CFG.q.participant) fields[CFG.q.participant] = ...`: конфиг модуля не имеет поля `participant` (service.js строит `CFG.q` только из `tg.cols`, ключей `participant` нет), ветка никогда не пишет. Поиск участника по таблице participants и createIfMissing при этом живут — не пишет только ссылку в строку сообщения.

**Зачем:** при переводе trytofly на модуль (PM-633) колонка «участник» (reqId 691) в trytofly используется — без ключа конфига связка потеряется. Лечение: добавить опциональный алиас `participant` в `tg.cols` (validateConfig TG_COLS + шаблон) или вырезать ветку. Пометка: docs/audit-2026-09-22.md, M8.

---
## TD-211: приглашения-ссылки — осознанные хвосты

**Файлы:** `backend/src/api/v2/modules/iam/service.js` (`registerWithInvite`), `frontend/src/views/workspaces/WorkspaceMembers.vue` (мёртвый email-диалог), `_v2_invitations.token` (plaintext).

**Хвосты:**

1. Гонка peek→accept в `registerWithInvite`: токен могут погасить между peek и accept — аккаунт остаётся без членства (оба факта названы в 409, не тихий успех). Кандидат на транзакционный peek-и-claim, пока оставлено как клиентская ситуация.
2. Мёртвый email-диалог в `WorkspaceMembers.vue`: `showInvite`/`doInvite` остались без кнопки вызова (вытеснены диалогом «Пригласить по ссылке»). Почистить при включении email-отправки приглашений.
3. Plaintext-токен в `_v2_invitations` — унаследован от email-приглашений. При ревизии переходить на hash для обеих механик сразу (email-приглашения и приглашения-ссылки), не только для новых.

**Приоритет:** Низкий — все три названы в ответах/интерфейсе, ущерба без отказа соседних механизмов нет.

---

## TD-212: fund-demo — снести таблицу «Машинка: участники» (10344) после приёмки

**Файлы:** воркспейс fund-demo, таблица 10344 (11 записей).

Машинка задач фонда (бот @assistent_fund_st_bot, план 2026-09-23-fund-machine-refactor.md) переведена на «Пользователи фонда» (6307, новые колонки «ID в PM» 13824 и «Формы имени» 13825). Таблица 10344 больше не читается ни одним правилом (проверено grep по правилам 10/12/13/14/15/16, 23.09), но оставлена до живой приёмки и суток без инцидентов.

**Что осталось:** `delete_table(10344)` в fund-demo — запись и её данные, продублированные в 6307. Предусловие: владелец подтвердил приёмку (текст/голос/ASR-шум/«не готово»//tasks/готово, assignee у созданной PM-задачи).

**Приоритет:** Низкий — таблица мёртвая, риска нет; снос необратим, поэтому только после приёмки.
