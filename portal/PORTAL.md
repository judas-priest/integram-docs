...
Шестизначный код письмом — для баз, где почта есть, а Telegram нет. Включается
одним полем конфига: `auth.emailReqId` — реквизит клиента, в котором лежит почта.
Без него роут отвечает `NOT_CONFIGURED`, а кнопка «Войти по почте» не показывается.

- письмо уходит тем же SMTP, которым платформа шлёт приглашения (`utils/mailer.js`);
  без `SMTP_HOST` код пишется в лог (dev-режим), как и код SMS без ключа провайдера;
- отправитель берётся из `notifications.emailFrom` конфига портала, а не из общего
  `SMTP_FROM`: письма воркспейса должны уходить с его домена, иначе SPF и DKIM
  этого домена не работают, а получатель видит в From чужое имя;
- ключ кода в Valkey несёт канал — `portal_otp:{db}:{channel}:{value}`. Иначе код,
  высланный на почту, подошёл бы к телефону с тем же значением.
...
- auth/request: 5 req/мин на IP и 3 req/10 мин на адресата (телефон или почту)
...
### Каталог: категория и «актуальность»

Категория товара — ref-колонка, а ref в EAV хранится инвертированно:
`(up=товар, t=<id категории>, val='<id колонки>')`. Прямое чтение
`cv.t = categoryReqId` не находит её никогда — карточки приезжали с
`category: null`, а `?category=` отдавал пустой список, даже когда категории
проставлены. Каталог читает оба паттерна: для инвертированного добавляется пара
join'ов, вторая — по `categoryTypeId`, чтобы под ссылку не попала любая строка,
где значение совпало с номером колонки (например цена 372).

| Ключ модуля `catalog` | Что |
|---|---|
| `categoryTypeId` | справочник категорий. Без него инвертированный паттерн не читается — проверять ссылку не по чему |
| `activeReqId` | флаг «продаём ли мы это». Задан — в выдачу попадает только помеченное (`1`/`true`/`да`/`yes`) |
| `hideOutOfStock` | прятать ли отсутствующее. По умолчанию `true` — как каталог вёл себя всегда. `false` — карточка остаётся в выдаче, витрина рисует «нет в наличии» и убирает кнопку корзины |

Связка ключей осмысленна вместе: `activeReqId` держит состав витрины, а
`hideOutOfStock: false` оставляет временно отсутствующее на месте — со своей
ссылкой, местом в поиске и честной плашкой вместо исчезновения.

`activeReqId` отвечает на другой вопрос, чем `inStockReqId`: наличие говорит
«есть на складе сейчас», актуальность — «продаём вообще». Без него состав
витрины держится на одном наличии: снимут галочку у живого товара — он пропадёт
из магазина, поставят у снятого с продажи — тот всплывёт. Не задан — поведение
прежнее.

### Позиция заказа и ссылка на товар

В `val` позиции лежит **название на момент заказа** — переименование карточки не
меняет состав старых заказов. Ссылка на саму карточку пишется отдельной
колонкой и нужна всему, что считает не по названию: списание склада, артикул в
сводах сборки, аналитика продаж.

Два необязательных ключа модуля `orders`:

| Ключ | Что |
|---|---|
| `itemProductReqId` | ref-колонка позиции на карточку товара. Не задан — ссылка не пишется, поведение прежнее |
| `itemProductTypeId` | тип карточек каталога. Задан — ссылкой становится только объект этого типа |

`productId` позиции приходит из тела запроса, то есть из браузера: перед записью
id проверяются одним запросом на существование и (если задан
`itemProductTypeId`) на принадлежность каталогу. Непрошедшие молча отбрасываются
— позиция создаётся без ссылки, а не роняет заказ. Оба пути, гостевой и
авторизованный, пишут одинаково.

### Фасовки товара

Фасовка — дочерняя запись карточки со своей ценой: «350 мл» и «1 л», «жидкий» и
«севший», набор в коробке и в подарочном ящике. Продаётся именно фасовка, и цена
карточки для такого товара — в лучшем случае одна из них.

Модуль `catalog`:

| Ключ | Что |
|---|---|
| `variantsTypeId` | тип дочерних записей-фасовок. Не задан — витрина прежняя, запроса за фасовками нет |
| `variantPriceReqId` | цена фасовки |
| `variantStockReqId` | остаток фасовки. Не задан — остаток «неизвестен», и витрина никого не прячет |

Фасовки отдаются только карточкой товара (`GET /portal/api/catalog/:slug`) —
в сетке каталога выбирать их негде, а запрос был бы на каждый товар списка.

Модуль `orders`:

| Ключ | Что |
|---|---|
| `itemVariantReqId` | колонка позиции под название фасовки |
| `itemVariantTypeId` | тип фасовок — по нему проверяется, что присланная фасовка есть **у этого** товара |

Название фасовки тоже приходит из браузера, поэтому пишется только после
проверки по дочерним записям товара; фасовка без опознанного товара не берётся
вовсе — проверять её не по чему.

Корзина (модуль `cart`, ключ `itemVariantReqId`) считает позицией пару
«товар + фасовка»: без этого 350 мл и 1 л схлопывались бы в одну строку с одной
ценой — и при добавлении, и при слиянии гостевой корзины после входа.
...
| POST | `/portal/api/auth/request` | Запросить OTP. Body: `{ phone }` (SMS) или `{ email }` (письмо) |
| POST | `/portal/api/auth/verify` | Проверить OTP, установить httpOnly cookie `portal_jwt`. Body: `{ phone, code }` или `{ email, code }` |
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/portal/api/staff/me` | Данные сотрудника (требует роль admin/owner/editor) |
| POST | `/portal/api/staff/items/:itemId/toggle` | Переключить флаг «Собрано» у позиции заказа |
| POST | `/portal/api/staff/orders/:id/items` | Добавить товар в заказ |
| GET | `/portal/api/staff/orders/:id/linked` | Заказы, связанные с этим заказом |
| POST | `/portal/api/staff/orders/:id/collect-all` | Отметить все позиции заказа собранными |
| POST | `/portal/api/staff/orders/:id/check-all-collected` | Проверить, собраны ли все позиции |
| POST | `/portal/api/staff/orders/merge` | Слить заказы-доноры в мастер-заказ |
| GET | `/portal/api/staff/events` | SSE-поток событий (обновления заказов, уведомления) |
| GET | `/portal/api/staff/product-variants` | Все варианты товаров, сгруппированные по товарам |
| GET | `/portal/api/staff/cdek/config` | Конфигурация CDEK-коннектора воркспейса |
| POST | `/portal/api/staff/orders/:id/cdek/calculate` | Расчёт тарифа CDEK для заказа |
| GET | `/portal/api/staff/cdek/pvz` | Поиск пунктов выдачи CDEK |
| POST | `/portal/api/staff/orders/:id/cdek/shipment` | Создать отправление CDEK |
| GET | `/portal/api/staff/orders/:id/cdek/label` | Скачать PDF-накладную CDEK |
...
| GET | `/portal/api/documents-list` | Блок-документы, видимые сотруднику (фильтр по грантам) |
| POST | `/portal/api/documents` | Создать блок-документ |
| PATCH | `/portal/api/documents/:docId` | Переименовать документ (только заголовок) |
| POST | `/portal/api/documents/:docId/blocks` | Добавить блок |
| PATCH | `/portal/api/documents/:docId/blocks/:blockId` | Править блок |
| DELETE | `/portal/api/documents/:docId/blocks/:blockId` | Удалить блок |
...
`agent.tools.allowWrite: true` включает не только write-инструменты, но и снимает белый список делегации для staff-запросов (`resolveRunDelegation` в `portal-agent-ctx.js` возвращает `allowedSlugs: null`) — роль с allowWrite это рабочее место с полным оркестратором, а не делегирующий агент.
