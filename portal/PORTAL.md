...
Возвращает `portal_jwt` с `user_type: 'portal_staff'` и TTL 8h (вместо 30d для клиентов).
...
| POST | `/portal/api/auth/staff/login` | Вход сотрудника (email+password, JWT 8h, user_type:'portal_staff') |
| POST | `/portal/api/orders/guest` | Заказ без регистрации. Body: `{ name, phone, items: [{productId, qty}] }` |
...
| GET | `/portal/api/tables/:typeId/objects` | Список записей EAV-таблицы (с пагинацией, фильтрацией) |
| GET | `/portal/api/tables/:typeId/objects/:id` | Одна запись |
| POST | `/portal/api/tables/:typeId/objects` | Создать запись |
| PATCH | `/portal/api/tables/:typeId/objects/:id` | Обновить запись |
| DELETE | `/portal/api/tables/:typeId/objects/:id` | Удалить запись |
...
| GET | `/portal/api/files/:filename` | Скачать файл по имени (только изображения) |
| GET | `/portal/api/reports/:reportId` | Данные отчёта (только отчёты из portal config, иначе 403) |
