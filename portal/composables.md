...
| `useCustomCodeApi.js` | API-фасад для custom_code компонентов. Оборачивает DataLayer: `useCollection`, `useRecord`, `useReport`, `useDocuments` (read), `useSchema`, `useObjects`, `useDocument`, `useTimeseries`, `useRelated` (extended data sources), `createRecord`, `updateRecord`, `deleteRecord`, `runConnector` (write), `apiFetch` (произвольный запрос к portal API), `invalidateCache(pattern)` (сброс кеша DataLayer), `shared`, `on`/`emit`, `getUser`, `isAuthenticated`, `toast`. Биндинги парсятся из формата `table:123`/`report:456` в структурированные объекты. Передаётся через prop `api` в компилированный SFC. |
