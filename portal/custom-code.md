...
| `api.useObjects(typeId, params?)` | ref | 200/мин | Записи произвольной таблицы (требует READ-грант) |
...
> Примечание: пример выше неточен — `useCollection` (как и `useRecord`/`useReport`/`useDocuments`) возвращает сам ref, а не объект с полем `data`; писать надо `const data = props.api.useCollection(...)`. Чинить на месте нельзя: живые компоненты уже написаны под текущее поведение — хвост TD-060, отдельная работа.

### Auth

| Метод | Описание |
|-------|----------|
| `api.getUser()` | Реактивный ref с записью клиента (EAV-поля). Первый вызов сам запускает чтение `/auth/me` в фоне — ref наполняется без ручного `refreshUser()` |
